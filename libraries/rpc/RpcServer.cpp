...
namespace hsrcore {
    namespace rpc {
        ...
        namespace detail
        {
            class RpcServerImpl : public hsrcore::rpc_stubs::CommonApiRpcServer
            {
            public:
                RpcServerConfig                                 _config;
                hsrcore::client::Client*                              _client;
                ...
                typedef std::map<std::string, hsrcore::api::MethodData> MethodMapType;
                ...
                RpcServerImpl(hsrcore::client::Client* client) :
                    _client(client),
                ...
                virtual hsrcore::api::CommonApi* get_client() const override;
                ...
                virtual void store_method_metadata(const hsrcore::api::MethodData& method_metadata);

                std::string help(const std::string& command_name) const;

                std::string make_short_description(const hsrcore::api::MethodData& method_data, bool show_decription = true) const
                {
                    std::string help_string;
                    std::stringstream short_description;
                    short_description << std::setw(100) << std::left;
                    help_string = method_data.name + " ";
                    for (const hsrcore::api::ParameterData& parameter : method_data.parameters)
                    {
                        if (parameter.classification == hsrcore::api::required_positional)
                            help_string += std::string("<") + parameter.name + std::string("> ");
                        else if (parameter.classification == hsrcore::api::optional_named)
                            help_string += std::string("{") + parameter.name + std::string("} ");
                        else if (parameter.classification == hsrcore::api::required_positional_hidden)
                            continue;
                        else
                            help_string += std::string("[") + parameter.name + std::string("] ");
                    }
                    ...
                    try {
                        if (_config.rpc_user.size())
                        {
                                ...
                                _config.rpc_password != password)
                            {
                                ...
                                s.add_header("WWW-Authenticate", "Basic realm=\"hsrcore wallet\"");
                ...
                fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s)
                {
                    ...
                    try {
                        ...
                        if (fc::time_point(hsrcore::blockchain::now()) - _last_cache_clear_time > fc::seconds(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC))
                        {
                            _last_cache_clear_time = hsrcore::blockchain::now();
                            _call_cache.clear();
                        }
				...
				bool ClientAllowed(const boost::asio::ip::address& address,vector<std::string> allow_ips)
				{
					if (allow_ips.size() == 0)
						return true;
					...
					if (address.is_v6()
						...
						return ClientAllowed(address.to_v6().to_v4(), allow_ips);
					...
					for(auto strAllow:allow_ips)
						if (WildcardMatch(strAddress.c_str(), strAllow.c_str()))
							return true;
					return false;
				}

                void accept_loop(vector<std::string> allow_ips)
                {
                    while (!_accept_loop_complete.canceled())
                    {
						...
						if (!ClientAllowed(boost::asio::ip::address::from_string(sock->remote_endpoint().get_address()) , allow_ips))
						{
							sock->close();
							continue;
						}
                ...
                void encrypted_accept_loop(const hsrcore::blockchain::PrivateKeyType& server_key, vector<std::string> allow_ips)
                {
                    while (!_encrypted_accept_loop_complete.canceled())
                    {
                        ...
                        try
                        {
							...
							if (!ClientAllowed(boost::asio::ip::address::from_string(sock->get_socket().remote_endpoint().get_address()), allow_ips))
							{
								sock->close();
								continue;
							}
                ...
                fc::variant dispatch_method_from_json_connection(fc::rpc::json_connection* con,
                    const hsrcore::api::MethodData& method_data,
                    const fc::variants& arguments)
                {
                    // ilog( "arguments: ${params}", ("params",arguments) );
                    if ((method_data.prerequisites & hsrcore::api::json_authenticated) &&
                ...
                fc::variant dispatch_authenticated_method(const hsrcore::api::MethodData& method_data,
                    const fc::variants& arguments_from_caller)
                {
                    ...
                    if (method_data.prerequisites & hsrcore::api::wallet_open)
                        verify_wallet_is_open();
                    if (method_data.prerequisites & hsrcore::api::wallet_unlocked)
                        verify_wallet_is_unlocked();
                    if (method_data.prerequisites & hsrcore::api::connected_to_network)
                        verify_connected_to_network();
                    if (method_data.prerequisites & hsrcore::api::sandbox_open)
                    ...
                    {
                        if (parameter.classification == hsrcore::api::required_positional
                            || parameter.classification == hsrcore::api::required_positional_hidden)
                        {
                        ...
                        else if (parameter.classification == hsrcore::api::optional_positional)
                        {
            ...
            hsrcore::api::CommonApi* RpcServerImpl::get_client() const
            {
                return _client;
            }
            ...
            void RpcServerImpl::store_method_metadata(const hsrcore::api::MethodData& method_metadata)
            {
            ...
            std::string RpcServerImpl::help(const std::string& command_name) const
            {
                ...
                { //display detailed description of requested command
                    ...
                    if (alias_iter != _alias_map.end())
                    {
                        ...
                        const hsrcore::api::MethodData& method_data = method_iter->second;
                        help_string += "Usage:\n";
        ...
        RpcServer::RpcServer(hsrcore::client::Client* client) :
        ...
        bool RpcServer::configure_rpc(const RpcServerConfig& cfg)
        {
            ...
            try
            {
                ...
                my->_accept_loop_complete = fc::async([=]{ my->accept_loop(cfg.rpc_allow_ip); }, "rpc_server accept_loop");
        ...
        bool RpcServer::configure_http(const RpcServerConfig& cfg)
        {
            ...
            try
            {
                ...
                while (!success) {
                    try
                    {
                        my->_httpd->listen(cfg.httpd_endpoint,cfg.http_allow_ip);
                        success = true;
                    }
                    catch (fc::exception& e)
                    {
        ...
        bool RpcServer::configure_encrypted_rpc(const RpcServerConfig& cfg)
        {
            ...
            try
            {
                ...
                my->_encrypted_accept_loop_complete = fc::async([=]{ my->encrypted_accept_loop(*server_key,cfg.rpc_allow_ip); }, "rpc_server accept_loop");
        ...
        const hsrcore::api::MethodData& RpcServer::get_method_data(const std::string& method_name)
        {
        ...
        std::vector<hsrcore::api::MethodData> RpcServer::get_all_method_data() const
        {
            std::vector<hsrcore::api::MethodData> result;
            for (const detail::RpcServerImpl::MethodMapType::value_type& value : my->_method_map)
                result.emplace_back(value.second);
            return result;
        }

        void RpcServer::register_method(hsrcore::api::MethodData data)
        {
