...
namespace hsrcore {
    namespace client {
        namespace detail {
            ...
            {
                return hsrcore::client::version_info();
            }
            ...
            variant_object ClientImpl::get_info()const
            {
                ...
                const auto share_entry = _chain_db->get_asset_entry(HSR_BLOCKCHAIN_SYMBOL);
                ...
                info["client_version"] = hsrcore::client::version_info()["client_version"].as_string();
            ...
            fc::variant_object ClientImpl::validate_address(const string& address) const
            {
                ...
                try
                {
                    hsrcore::blockchain::PublicKeyType test_key(address);
                    result["isvalid"] = true;
                }
                ...
                const std::vector<fc::variants>& parameters_list) const
            {
                ...
                FC_ASSERT(_self->get_rpc_server()->get_method_data(method_name).prerequisites == hsrcore::api::no_prerequisites,
