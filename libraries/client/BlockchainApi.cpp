    ...
    namespace client {
        namespace detail {
            ...
            vector<AccountEntry> ClientImpl::blockchain_list_active_delegates(uint32_t first, uint32_t count)const
            {
                ...
                if (first > 0) --first;
                FC_ASSERT(first < HSR_BLOCKCHAIN_NUM_DELEGATES, "Only the first 99 delegates are active");
                FC_ASSERT(first + count <= HSR_BLOCKCHAIN_NUM_DELEGATES, "Only the first 99 delegates are active");
                vector<AccountIdType> all_delegate_ids = _chain_db->get_active_delegates();
                FC_ASSERT(all_delegate_ids.size() == HSR_BLOCKCHAIN_NUM_DELEGATES, "The num of active delegates should equal to HSR_BLOCKCHAIN_NUM_DELEGATES");
            ...
            vector<string> ClientImpl::blockchain_list_missing_block_delegates(uint32_t block_num)
            {
                ...
                timestamp += HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                while (timestamp != this_block.timestamp)
                    // if time interval between prev_block and this block greater than HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
            ...
            vector<HSRTrxidBalance> ClientImpl::blockchain_get_hsr_account_balance_entry(const uint32_t block_num)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

                uint32_t last_scan_num = _wallet->get_last_scanned_block_number_for_hsr();
                auto results = _chain_db->fetch_hsr_full_entry(block_num, last_scan_num);
            ...
            std::vector<std::pair<hsrcore::blockchain::TransactionIdType, hsrcore::blockchain::SignedTransaction>>  ClientImpl::blockchain_list_pending_transactions() const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");

                std::vector<std::pair<hsrcore::blockchain::TransactionIdType, hsrcore::blockchain::SignedTransaction>> trxs;
            ...
            oAccountEntry detail::ClientImpl::blockchain_get_account(const string& account)const
            {
                ...
                try
                {
                    ...
                    else if (account.substr(0, string(HSR_ADDRESS_PREFIX).size()) == HSR_ADDRESS_PREFIX) {
                ...
                uint32_t trx_type, const std::string& call_method)const
            {
                ...
                FC_ASSERT((trx_type == 99) ||(trx_type >= hsrcore::blockchain::TransactionType::register_contract_transaction &&
                    trx_type <= hsrcore::blockchain::TransactionType::call_contract_transaction), "invalid transaction type");
                    ...
                    for (; block_num_start <= block_num_end; ++block_num_start)
                    {
                        ...
                        for (auto trx_pair : trx_map){
                            ...
                            if ((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type.value == transaction_op_type))
                            {
                                ...
                                if (trx_type == hsrcore::blockchain::TransactionType::call_contract_transaction)
                                {
                                    if (call_method != "")
                                    {
                                ...
                                if (summary.trx_type == hsrcore::blockchain::TransactionType::call_contract_transaction)
                                {
			...
			std::vector<fc::string> ClientImpl::blockchain_list_pub_all_address(const std::string& pub_key) const
			{
				...
				try
				{
					...
					if (fc::string::npos != pub_key.find(HSR_ADDRESS_PREFIX))
					{
						auto pub_type = hsrcore::blockchain::PublicKeyType(pub_key);
            ...
            variant_object ClientImpl::blockchain_get_info()const
            {
                ...
                info["name"] = HSR_BLOCKCHAIN_NAME;
                info["symbol"] = HSR_BLOCKCHAIN_SYMBOL;
                info["address_prefix"] = HSR_ADDRESS_PREFIX;
                info["db_version"] = HSR_BLOCKCHAIN_DATABASE_VERSION;
                info["genesis_timestamp"] = _chain_db->get_genesis_timestamp();

                info["block_interval"] = HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                info["delegate_num"] = HSR_BLOCKCHAIN_NUM_DELEGATES;
                ...
                info["name_size_max"] = HSR_BLOCKCHAIN_MAX_NAME_SIZE;
                info["memo_size_max"] = HSR_BLOCKCHAIN_MAX_MESSAGE_SIZE;
                info["data_size_max"] = HSR_BLOCKCHAIN_MAX_NAME_DATA_SIZE;

                info["symbol_size_max"] = HSR_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE;
                info["symbol_size_min"] = HSR_BLOCKCHAIN_MIN_SYMBOL_SIZE;
                info["asset_shares_max"] = HSR_BLOCKCHAIN_MAX_SHARES;
                info["short_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(HSR_BLOCKCHAIN_MIN_SYMBOL_SIZE);
                info["long_symbol_asset_reg_fee"] = _chain_db->get_asset_registration_fee(HSR_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE);

                info["statistics_enabled"] = _chain_db->get_statistics_enabled();

                info["relay_fee"] = _chain_db->get_relay_fee();
                info["max_pending_queue_size"] = HSR_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
                info["max_trx_per_second"] = HSR_BLOCKCHAIN_MAX_TRX_PER_SECOND;
                ...
                if (fp != NULL)
                {
                    while (true)
                    {
                        ...
                        if (fgets(c, 256, fp) != NULL)
                        {
                            ...
                            std::string allstring = (std::string)hsrcore::blockchain::Address(hsrcore::blockchain::PtsAddress(pow_address));
                            allstring += balance;
            ...
            hsrcore::blockchain::BlockchainSecurityState ClientImpl::blockchain_get_security_state()const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
                ...
                state.estimated_confirmation_seconds = (uint32_t)(required_confirmations * HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
                state.participation_rate = participation_rate;
                if (!blockchain_is_synced())
                {
                    state.alert_level = hsrcore::blockchain::BlockchainSecurityState::grey;
                }
                else if (required_confirmations <= HSR_BLOCKCHAIN_NUM_DELEGATES / 2
                    && participation_rate > 80)
                {
                    state.alert_level = hsrcore::blockchain::BlockchainSecurityState::green;
                }
                else if (required_confirmations > HSR_BLOCKCHAIN_NUM_DELEGATES
                    || participation_rate < 60)
                {
                    state.alert_level = hsrcore::blockchain::BlockchainSecurityState::red;
                }
                else
                {
                    state.alert_level = hsrcore::blockchain::BlockchainSecurityState::yellow;
                }
            ...
            bool ClientImpl::blockchain_is_synced() const
            {
                ...
                return (blockchain::now() - _chain_db->get_head_block().timestamp) < fc::seconds(1000 * HSR_PRODUCTE_BLOCK_INTERAL);
            }
            ...
            std::string ClientImpl::blockchain_export_fork_graph(uint32_t start_block, uint32_t end_block, const hsrcore::blockchain::FilePath& filename)const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            bool ClientImpl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
            {
                ...
                catch (...)
                {
                    try {
                        ...
                        // NB LHS of == operator is hsrcore::blockchain::public_key_type and RHS is fc::ecc::public_key,
            ...
            hsrcore::blockchain::Asset ClientImpl::blockchain_unclaimed_genesis() const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
