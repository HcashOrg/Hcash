...
//#include <hsrcore/mail/server.hpp>
...
namespace hsrcore {
    namespace client {
        ...
        const string HSR_MESSAGE_MAGIC = "HSR Signed Message:\n";
        ...
        program_options::variables_map parse_option_variables(int argc, char** argv)
        {
            ...
            option_config.add_options()
				...
				("rpcallowip", program_options::value<string>(), "Set allowance ip for JSON-RPC connections")
				...
				("httpallowip", program_options::value<string>(), "Set allowance ip for  HTTP JSON-RPC connections")
                ...
                ("growl-identifier", program_options::value<std::string>(), "A name displayed in growl messages to identify this hsr_client instance")
				("staking", program_options::value<bool>()->default_value(false),"Start POS mining in background")
                ;
            ...
            {
                std::cout << fc::json::to_pretty_string(hsrcore::client::version_info()) << "\n";
                exit(0);
            }
        ...
        fc::path get_data_dir(const program_options::variables_map& option_variables)
        {
            try {
                ...
                else
                {
                    ...
                    datadir = fc::app_path() / get_os_specific_dir_name(HSR_BLOCKCHAIN_NAME);
                }
                return datadir;
        ...
        void load_checkpoints(const fc::path& data_dir)
        {
            try {
                ...
                decltype(hsrcore::blockchain::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                ...
                {
                    decltype(hsrcore::blockchain::detail::CHECKPOINT_BLOCKS) external_checkpoints;
                    if (hsrcore::blockchain::detail::CHECKPOINT_BLOCKS.empty() || external_checkpoints.crbegin()->first >= hsrcore::blockchain::detail::CHECKPOINT_BLOCKS.crbegin()->first)
                    {
                        ulog("Using blockchain checkpoints from file: ${x}", ("x", checkpoint_file.preferred_string()));
                        hsrcore::blockchain::detail::CHECKPOINT_BLOCKS = external_checkpoints;
                        return;
                    }
                }
                ...
                if (!file_exception.valid())
                {
                    ...
                    fc::json::save_to_file(hsrcore::blockchain::detail::CHECKPOINT_BLOCKS, checkpoint_file);
                }
                else
                {
                ...
                if (!hsrcore::blockchain::detail::CHECKPOINT_BLOCKS.empty())
        ...
        namespace detail
        {
                ...
                const program_options::variables_map& option_variables)
            {
                if (option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable)
                {
                    ...
                    // hsrcore::rpc::rpc_server::config rpc_config(cfg.rpc);
					...
					if (option_variables.count("rpcallowip"))
						boost::split(cfg.rpc.rpc_allow_ip, option_variables["rpcallowip"].as<string>(), boost::is_any_of(","), token_compress_on);
					if (option_variables.count("httpallowip"))
						boost::split(cfg.rpc.http_allow_ip, option_variables["httpallowip"].as<string>(),boost::is_any_of( ","), token_compress_on);
					if (option_variables.count("rpcport"))
						cfg.rpc.rpc_endpoint.set_port(option_variables["rpcport"].as<uint16_t>());
            ...
            void ClientImpl::start_delegate_loop()
            {
                ...
                if (!_time_discontinuity_connection.connected())
                    _time_discontinuity_connection = hsrcore::blockchain::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
            ...
            void ClientImpl::delegate_loop()
            {
                ...
                {
                    set_target_connections(HSR_NET_DELEGATE_DESIRED_CONNECTIONS);
                ...
                if (next_block_time.valid())
                {
                    ...
                    if (*next_block_time <= now)
                    {
                        try
                        {
                            ...
                            FC_ASSERT((now - *next_block_time) < fc::seconds(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                                "You missed your slot at time: ${t}!", ("t", *next_block_time));

                            FullBlock next_block = _chain_db->generate_block(*next_block_time,_wallet->get_my_delegates().begin()->owner_key, _delegate_config);
            ...
            void ClientImpl::start_rebroadcast_pending_loop()
            {
                if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
                    "rebroadcast_pending");
            }
            void ClientImpl::start_fork_update_time_loop()
            {
                if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
                    ...
                    fc::time_point::now() + fc::seconds((int64_t)(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
                    "rebroadcast_pending");
            }
            ...
            hsrcore::blockchain::PublicKeyType ClientImpl::wallet_get_account_owner_publickey(const std::string& account_name)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::rebroadcast_pending_loop()
            {
                ...
                else
                {
                    try
                    {
                        ...
                        std::vector<hsrcore::blockchain::SignedTransaction> temp_vec;
                    ...
                    try
                    {
                        ...
                        if (fork_data && fork_data->is_known)
                        {
                            if (sync_mode && !fork_data->is_linked)
                                FC_THROW_EXCEPTION(hsrcore::blockchain::unlinkable_block,
                        ...
                        else
                        {
                            ...
                            if (sync_mode && !result.is_linked)
                                FC_THROW_EXCEPTION(hsrcore::blockchain::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
            ...
            bool ClientImpl::has_item(const hsrcore::net::ItemId& id)
            {
            ...
            */
            bool ClientImpl::handle_message(const hsrcore::net::Message& message_to_handle, bool sync_mode)
            {
                if (g_client_quit)
                    return true;
                ...
                try
                {
                    ...
                    case block_message_type:
                    {
                        ...
                        if (block_message_to_handle.block.block_size() > HSR_BLOCKCHAIN_MAX_BLOCK_SIZE)
                        {
                        ...
                        hsrcore::blockchain::BlockIdType old_head_block = _chain_db->get_head_block_id();
                    ...
                    case trx_message_type:
                    {
                        ...
                        if (trx_message_to_handle.trx.data_size() > HSR_BLOCKCHAIN_MAX_TRX_SIZE)
                        {
                ...
                catch (const hsrcore::blockchain::insufficient_relay_fee& original_exception)
                {
                    FC_THROW_EXCEPTION(hsrcore::net::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
                        ("original_exception", original_exception.to_detail_string()));
                }
                catch (const hsrcore::blockchain::block_older_than_undo_history& original_exception)
                {
                    FC_THROW_EXCEPTION(hsrcore::net::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
                        ("original_exception", original_exception.to_detail_string()));
                }
            }
            ...
            */
            std::vector<hsrcore::net::ItemHashType> ClientImpl::get_item_ids(uint32_t item_type,
                const std::vector<hsrcore::net::ItemHashType>& blockchain_synopsis,
                uint32_t& remaining_item_count,
                uint32_t limit /* = 2000 */)
            {
                // limit = 20; // for testing
                FC_ASSERT(item_type == hsrcore::client::block_message_type);
                ...
                hsrcore::net::ItemHashType last_seen_block_hash;
                for (const hsrcore::net::ItemHashType& item_hash : boost::adaptors::reverse(blockchain_synopsis))
                {
                    try
                    {
                ...
                std::vector<hsrcore::net::ItemHashType> hashes_to_return;
            ...
            hsrcore::net::Message ClientImpl::get_item(const hsrcore::net::ItemId& id)
            {
                if (id.item_type == block_message_type)
                {
                    //   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
                    hsrcore::client::BlockMessage block_message_to_send(_chain_db->get_block(id.item_hash));
            ...
            fc::path ClientImpl::compile_script(const fc::path& filename) const
            {
                ...
                try{
                    ...
                    {
                        FC_THROW_EXCEPTION(hsrcore::blockchain::invalid_script_source_filename, "script source file name should end with .lua or .glua");
                    }
                    ...
                    {
                        FC_THROW_EXCEPTION(hsrcore::blockchain::save_bytecode_to_scriptfile_fail, err_msg);
                    }
            ...
            uint32_t ClientImpl::get_block_number(const hsrcore::net::ItemHashType& block_id)
            {
                return _chain_db->get_block_num(block_id);
            }

            fc::time_point_sec ClientImpl::get_block_time(const hsrcore::net::ItemHashType& block_id)
            {
                if (block_id == hsrcore::net::ItemHashType())
                {
            ...
            fc::time_point_sec ClientImpl::get_blockchain_now()
            {
                ...
                return hsrcore::blockchain::nonblocking_now();
            }

            hsrcore::net::ItemHashType ClientImpl::get_head_block_id() const
            {
            ...
            {
                return hsrcore::blockchain::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
            }
            ...
            void ClientImpl::blocks_too_old_monitor_task()
            {
                ...
                if (_chain_db->get_head_block().timestamp < hsrcore::blockchain::now() - fc::seconds(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
                    !_sync_mode &&
                ...
                if (!_blocks_too_old_monitor_done.canceled())
                    ...
                    fc::time_point::now() + fc::seconds(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
                    "block_monitor_task");
            }
        ...
        Client::Client(const std::string& user_agent,
            hsrcore::net::SimulatedNetworkPtr network_to_connect_to)
            : my(new detail::ClientImpl(this, user_agent))
        {
            ...
            const std::function<void(float)> replay_status_callback)
        {
            try {
                ...
                my->_wallet = std::make_shared<hsrcore::wallet::Wallet>(my->_chain_db, my->_config.wallet_enabled);
                my->_wallet->set_data_directory(wallet_dir / "wallets");

                /*  //delete?
                if (my->_config.mail_server_enabled)
                {
                my->_mail_server = std::make_shared<hsrcore::mail::server>();
                my->_mail_server->open(data_dir / "mail");
                }
                my->_mail_client = std::make_shared<hsrcore::mail::client>(my->_wallet, my->_chain_db);
        ...
        */
        ...
        hsrcore::rpc::RpcServerPtr Client::get_rpc_server()const { return my->_rpc_server; }
        hsrcore::net::NodePtr Client::get_node()const { return my->_p2p_node; }

        fc::variant_object version_info()
        {
            string client_version(hsrcore::utilities::git_revision_description);
            const size_t pos = client_version.find('/');
            if (pos != string::npos && client_version.size() > pos)
                client_version = client_version.substr(pos + 1);
#ifdef HSR_TEST_NETWORK
            //client_version += "-testnet-" + std::to_string(HSR_TEST_NETWORK_VERSION);
            client_version += "-testnet";
#endif

            fc::mutable_variant_object info;
            info["blockchain_name"] = HSR_BLOCKCHAIN_NAME;
            info["blockchain_description"] = HSR_BLOCKCHAIN_DESCRIPTION;
            info["client_version"] = client_version;
            info["hsr_revision"] = hsrcore::utilities::git_revision_sha;
            info["hsr_revision_age"] = fc::get_approximate_relative_time_string(fc::time_point_sec(hsrcore::utilities::git_revision_unix_timestamp));
...
#if defined(HSR_CLIENT_JENKINS_BUILD_NUMBER)
            info["jenkins_build_number"] = HSR_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(HSR_CLIENT_JENKINS_BUILD_URL)
            info["jenkins_build_url"] = HSR_CLIENT_JENKINS_BUILD_URL;
#endif
        ...
        void Client::start_networking(std::function<void()> network_started_callback)
        {
            ...
            {
                hsrcore::net::ChainDownloader* chain_downloader = new hsrcore::net::ChainDownloader();
        ...
        void Client::configure_from_command_line(int argc, char** argv)
        {
            ...
            {
                my->_cli = new hsrcore::cli::Cli(this, nullptr, &std::cout);
                return;
            }
            g_client = this;
            // parse command-line options
            ...
            // else we use the default set in hsrcore::net::node
            ...
            else //we will accept input from the console
            {
                ...
                if (option_variables.count("log-commands") <= 0)
                {
                    ...
                    my->_cli = new hsrcore::cli::Cli(this, my->_command_script_holder.get(), &std::cout);
                }
                else
                {
                    ...
                    my->_cli = new hsrcore::cli::Cli(this, my->_command_script_holder.get(), my->_tee_stream.get());
            ...
            if (option_variables["upnp"].as<bool>())
            {
                ...
                my->_upnp_service = std::unique_ptr<hsrcore::net::UpnpService>(new hsrcore::net::UpnpService);
            ...
            if (option_variables.count("growl") || my->_config.growl_notify_endpoint)
            {
                ...
                std::string hsr_instance_identifier = "HSR";
                if (option_variables.count("growl-identifier"))
                    hsr_instance_identifier = option_variables["growl-identifier"].as<std::string>();
                else if (my->_config.growl_hsr_client_identifier)
                    hsr_instance_identifier = *my->_config.growl_hsr_client_identifier;
                my->_notifier = std::make_shared<HSRGntpNotifier>(host_to_notify, port_to_notify, hsr_instance_identifier, growl_password);
			...
			if (option_variables.count("staking"))
			{
				my->fposgenerate = option_variables["staking"].as<bool>();
				if(option_variables["staking"].as<bool>())
					my->GenerateStakes();
			}
            ...
            {
                my->_chain_server = std::unique_ptr<hsrcore::net::ChainServer>(
                    new hsrcore::net::ChainServer(my->_chain_db,
        ...
        hsrcore::net::NodeIdType Client::get_node_id() const
        {
        ...
        void Client::add_node(const string& remote_endpoint, int32_t oper_flag)
        {
            ...
            catch (const hsrcore::net::already_connected_to_requested_peer&)
            {
            }
        }
        ...
        {
            hsrcore::net::ItemId head_item_id;
            head_item_id.item_type = hsrcore::client::block_message_type;
            uint32_t last_block_num = my->_chain_db->get_head_block_num();
            if (last_block_num == (uint32_t)-1)
                head_item_id.item_hash = hsrcore::net::ItemHashType();
            else
                head_item_id.item_hash = my->_chain_db->get_head_block_id();
            my->_p2p_node->sync_from(head_item_id, hsrcore::blockchain::get_list_of_fork_block_numbers());
        ...
        bool Client::handle_message(const hsrcore::net::Message& message, bool sync_mode)
        {
        ...
        */
        ...
        hsrcore::api::CommonApi* Client::get_impl() const
        {
            return my.get();
        }
