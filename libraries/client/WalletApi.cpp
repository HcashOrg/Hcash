...
#include "hshare_wallet/hshare_addr_util.h"
...
namespace hsrcore {
    namespace client {
        namespace detail {
            ...
            std::vector<hsrcore::blockchain::AccountEntry> ClientImpl::wallet_get_all_approved_accounts(int8_t approval)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            void detail::ClientImpl::wallet_create(const string& wallet_name, const string& password, const string& brain_key)
            {
                ...
                if (brain_key.size() && brain_key.size() < HSR_WALLET_MIN_BRAINKEY_LENGTH) FC_CAPTURE_AND_THROW(brain_key_too_short);
                if (password.size() < HSR_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            void read_keys(const fc::variant& vo, vector<PrivateKeyType>& keys, const string& password)
            {
                ...
                try
                {
                    ...
                    const auto key = hsrcore::utilities::wif_to_key(wif_key);
                ...
                catch (const hsrcore::wallet::invalid_password&)
                {
                    throw;
                }
                catch (...)
            ...
            */
            ...
            vector<hsrcore::wallet::PrettyTransaction> detail::ClientImpl::wallet_transaction_history_splite(const std::string& account_name,
                ...
                int32_t transaction_type)const
            {
                ...
                try {
                    const auto history = _wallet->get_pretty_transaction_history(account_name, 0, -1, asset_symbol, (hsrcore::wallet::Wallet::TrxType)transaction_type, true);
			...
			WalletTransactionEntry detail::ClientImpl::sign_build_transaction(const hsrcore::wallet::WalletTransactionEntry& trasaction_building)
			{
			...
			bool  detail::ClientImpl::broadcast_building_transaction(const hsrcore::wallet::WalletTransactionEntry& trasaction_building)
			{
				WalletTransactionEntry trx_entry = trasaction_building;
            ...
            vector<hsrcore::wallet::AccountAddressData> detail::ClientImpl::wallet_list_my_addresses() const
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
			...
			bool detail::ClientImpl::wallet_import_hshare_private_key(const std::string& acc_name, const FilePath & infile)
            {
				...
				for (auto wif : vwif)
				{
					...
					std::string name = acc_name + ss.str();

					printf("account name %s\n", name.c_str());
					name =  detail::ClientImpl::wallet_import_private_key(wif, name, true);
					printf("import account %s\n", name.c_str());
				}
            ...
            // vector<hsrcore::wallet::escrow_summary> ClientImpl::wallet_escrow_summary( const string& account_name ) const
            // { try {
            ...
            std::map<std::string, hsrcore::blockchain::DelegatePaySalary>  ClientImpl::wallet_active_delegate_salary()
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            void ClientImpl::wallet_set_transaction_imessage_fee_coe(const string& fee_coe)
            {
                ...
                try
                {
                    ...
                    if (ipos != string::npos)
                    {
                        ...
                        FC_ASSERT((precision_input <= HSR_BLOCKCHAIN_PRECISION), "Precision is not correct");
                    }
                    double dFee = std::stod(fee_coe);
                    _wallet->set_transaction_imessage_fee_coe(static_cast<int64_t>(floor(dFee * HSR_BLOCKCHAIN_PRECISION + 0.5)));
                }
            ...
            double ClientImpl::wallet_get_transaction_imessage_fee_coe()
            {
                ...
                return ((double)fee_coe) / HSR_BLOCKCHAIN_PRECISION;
            }
            ...
            Asset ClientImpl::wallet_get_transaction_fee(const string& fee_symbol)
            {
                ...
                if (fee_symbol.empty())
                    return _wallet->get_transaction_fee(_chain_db->get_asset_id(HSR_BLOCKCHAIN_SYMBOL));
            ...
            std::string ClientImpl::wallet_transfer_to_address_rpc(const std::string& amount_to_transfer, const std::string& asset_symbol, const std::string& from_account_name, const std::string& to_address, const hsrcore::blockchain::Imessage& memo_message /* = fc::json::from_string("").as<std::string>() */, const hsrcore::wallet::VoteStrategy& strategy /* = fc::json::from_string("vote_recommended").as<hsrcore::wallet::vote_strategy>() */)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            std::string detail::ClientImpl::wallet_transfer_to_public_account_rpc(
                ...
                const string& from_account_name,
                const string& to_account_name,
                const hsrcore::blockchain::Imessage& memo_message,
                const hsrcore::wallet::VoteStrategy& strategy)
            {
                // set limit in  sandbox state
                if (_chain_db->get_is_in_sandbox())
                    FC_THROW_EXCEPTION(sandbox_command_forbidden, "in sandbox, this command is forbidden, you cannot call it!");
            ...
            string ClientImpl::wallet_account_balance_rpc(const string& account_name)const
            {
                ...
                try {
                    ...
                    else
                    {
                        ...
                        auto resu = re_val.find(hsrcore::blockchain::Asset(0, 0).asset_id);
                        if (resu == re_val.end())
                        {
                            res += "0\"}";
                        }
