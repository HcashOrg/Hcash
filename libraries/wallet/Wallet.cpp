    ...
    namespace wallet {
        ...
        namespace detail {
            ...
            WalletImpl::WalletImpl()
            {
				...
				_main_scan_thread = std::unique_ptr<fc::thread>(new fc::thread("wallet_main_scan_process"));
            }
            ...
            void WalletImpl::state_changed(const PendingChainStatePtr& state)
            {
                ...
                if (_blockchain->get_head_block_num() < last_unlocked_scanned_number)
                {
                    ...
                    self->set_last_scanned_block_number_for_hsr(_blockchain->get_head_block_num());
                }
            }
            ...
            void WalletImpl::block_applied(const BlockSummary& summary)
            {
                ...
                if ((t - _blockchain->get_head_block_timestamp()) <= fc::seconds(HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2))
                ...
                try {
                    ...
                    try
                    {
                        ...
                        while (current_block_num <= head_block_num && count < limit && !_scan_in_progress.canceled())
                        {
                            ...
                            self->set_last_scanned_block_number_for_hsr(prev_block_num);
                            ++current_block_num;
            ...
            void WalletImpl::upgrade_version()
            {
                ...
                if (current_version > HSR_WALLET_VERSION)
                {
                    FC_THROW_EXCEPTION(unsupported_version, "Wallet version newer than client supports!",
                        ("wallet_version", current_version)("supported_version", HSR_WALLET_VERSION));
                }
                else if (current_version == HSR_WALLET_VERSION)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    {
                        self->set_transaction_fee(Asset(HSR_WALLET_DEFAULT_TRANSACTION_FEE));
                        self->set_transaction_imessage_fee_coe(HSR_BLOCKCHAIN_MIN_MESSAGE_FEE_COE);
                        self->set_transaction_imessage_soft_max_length(HSR_BLOCKCHAIN_MAX_SOFT_MAX_MESSAGE_SIZE);
                    }

                    if (current_version < 106)
                    {
                        self->set_transaction_expiration(HSR_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC);
                    }
                    ...
                    {
                        self->set_last_scanned_block_number_for_hsr(0);
                    }
                    if (_unlocked_upgrade_tasks.empty())
                    {
                        self->set_version(HSR_WALLET_VERSION);
                ...
                const string& brainkey)
            {
                try {
                    ...
                    if (password.size() < HSR_WALLET_MIN_PASSWORD_LENGTH)
             ...
             *  returned will contain no more than HSR_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
             */
            SlateEntry WalletImpl::get_delegate_slate(const VoteStrategy strategy)const
            {
                ...
                {
                    slate_size = std::min<size_t>(HSR_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size());
                }
                else if (strategy == vote_random)
                {
                    slate_size = std::min<size_t>(HSR_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size());
                    slate_size = rand() % (slate_size + 1);
                }
                else if (strategy == vote_recommended && for_candidates.size() < HSR_BLOCKCHAIN_MAX_SLATE_SIZE)
                {
                    ...
                    while (for_candidates.size() < HSR_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0)
                    {
                        int best_rank = 0;
                ...
                FC_ASSERT(entry.slate.size() <= HSR_BLOCKCHAIN_MAX_SLATE_SIZE);
            ...
            const string& brainkey)
        {
            try {
                ...
                try
                {
                    ...
                    open(wallet_name);
                    unlock(password, HSR_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::create_from_json(const path& filename, const string& wallet_name, const string& passphrase)
        {
            try {
                ...
                try
                {
                    ...
                    close();
                    open(wallet_name);
                    unlock(passphrase, HSR_WALLET_DEFAULT_UNLOCK_TIME_SEC);
                }
                catch (...)
                {
        ...
        void Wallet::unlock(const string& password, uint32_t timeout_seconds)
        {
            try {
                ...
                try
                {
                    ...
                    const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - hsrcore::blockchain::now().sec_since_epoch();
        ...
        bool Wallet::check_passphrase(const string& passphrase)
        {
            try {
                ...
                if (passphrase.size() < HSR_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
        ...
        void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
        {
            try {
                if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
                if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
                if (new_passphrase.size() < HSR_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
            ...
            const variant& private_data)
        {
            try {
                ...
                if (num_accounts_before == 0)
                {
                    ...
                    set_last_scanned_block_number_for_hsr(0);
                }
            ...
            bool create_account)
        {
            try {
                ...
                auto key = hsrcore::utilities::wif_to_key(wif_key);
                if (key.valid())
                {
        ...
        void Wallet::start_scan(const uint32_t start_block_num, const uint32_t limit)
        {
            try {
                ...
                my->_scan_in_progress = my->_main_scan_thread->async(scan_chain_task, "scan_chain_task");
        ...
        VoteSummary Wallet::get_vote_status(const string& account_name)
        {
            ...
            for (const auto& item : items)
            {
                ...
                for (const auto& entry : entrys)
                {
                    if (entry.asset_id() != 0)
                        continue;
                    if (entry.slate_id() != my_slate.id() && entry.balance > 1 * HSR_BLOCKCHAIN_PRECISION) // ignore dust
                    ...
                    total_possible += entry.get_spendable_balance(my->_blockchain->now()).amount * HSR_BLOCKCHAIN_MAX_SLATE_SIZE;
                }
            }
		...
		uint64_t Wallet::GetStakeWeight()
		{
			uint64_t nWeight = 0;
			auto all_account_entrys = get_spendable_account_balance_entrys();
		...
		bool Wallet::CheckWork(FullBlock& pblock)
		{
			try {
				...
				// Found a solution
				{
					...
					if (pblock.block_num == 1)
					{
						FC_ASSERT(pblock.previous == BlockIdType(), "CheckWork() : generated block is stale");
					}
					else
						FC_ASSERT(pblock.previous == my->_blockchain->get_head_block().id(), "CheckWork() : generated block is stale");
				}
		...
		void Wallet::pow_sign_block(SignedBlockHeader& header)const
		{
			try {
				if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
				if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
				...
				const PublicKeyType public_signing_key = sign_account->owner_key;
            ...
            bool sign)
        {
            try {
                ...
                const auto version = hsrcore::client::version_info()["client_version"].as_string();
        ...
        void Wallet::repair_entrys(const optional<string>& collecting_account_name)
        {
            try {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (name.find(HSR_ADDRESS_PREFIX) != 0)
                        continue;
        ...
        std::map<std::string, hsrcore::blockchain::DelegatePaySalary> Wallet::query_delegate_salarys()
        {
            FC_ASSERT(is_open());
            FC_ASSERT(is_unlocked());
            std::map<std::string, hsrcore::blockchain::DelegatePaySalary> Salary_Map;
        ...
        WalletTransactionEntry Wallet::register_contract(const string& owner, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
        {
            ...
            {
                FC_THROW_EXCEPTION(hsrcore::blockchain::invalid_contract_filename, "contract bytecode file name should end with .gpc");
            }
            ...
            FC_ASSERT(asset_symbol == HSR_BLOCKCHAIN_SYMBOL, "asset_symbol must be HSR");

            const auto asset_rec = data_ptr->get_asset_entry(asset_symbol);
            FC_ASSERT(asset_rec.valid(), "Asset not exist!");
            FC_ASSERT(asset_rec->id == 0, "asset_symbol must be HSR");
            ...
            FC_ASSERT(register_fee.asset_id == 0, "register fee must be HSR");
            FC_ASSERT(margin.asset_id == 0, "register fee must be HSR");
            FC_ASSERT(fee.asset_id == 0, "register fee must be HSR");
            trx.expiration = blockchain::now() + get_transaction_expiration();
            my->sign_transaction(trx, required_signatures);
		...
		WalletTransactionEntry Wallet::register_contract_without_signature(const string& owner_public_key_str, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing)
		{
			ChainInterfacePtr data_ptr = get_correct_state_ptr();
			...
			FC_ASSERT(asset_rec.valid(), "Asset not exist!");
			FC_ASSERT(asset_rec->id == 0, "asset_symbol must be HSR");
			FC_ASSERT(!my->_blockchain->get_is_in_sandbox(), "not support sandbox");
			PublicKeyType  owner_public_key = PublicKeyType(owner_public_key_str);
			...
			if (!is_testing)
			{
				get_enough_balances_by_address(owner_public_key_str, asset_limit + register_fee + fee + margin, balances, required_signatures);
			}
			else
				required_signatures.insert(owner_address);
        ...
        std::vector<hsrcore::blockchain::Asset> Wallet::register_contract_testing(const string& owner, const fc::path codefile)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(owner), "Invalid account name");
            ...
            auto trans_entry = register_contract(owner,
                codefile,
                HSR_BLOCKCHAIN_SYMBOL,
                (double)asset_for_exec.amount / HSR_BLOCKCHAIN_PRECISION, 
                true
                );
            ...
            std::vector<hsrcore::blockchain::Asset> asset_vec;
		...
		hsrcore::wallet::WalletTransactionEntry Wallet::call_contract(const string caller, const ContractIdType contract, const string method, const string& arguments, const string& asset_symbol, double cost_limit, bool is_testing)
		{
			ChainInterfacePtr data_ptr = get_correct_state_ptr();
		...
		WalletTransactionEntry Wallet::call_contract_without_signature(const string caller_public_key_str, const ContractIdType contract, const string method, const string& arguments, const string& asset_symbol, double cost_limit, bool is_testing)
		{
			ChainInterfacePtr data_ptr = get_correct_state_ptr();
			...
			FC_ASSERT(asset_rec.valid(), "Asset not exist!");
			FC_ASSERT(asset_rec->id == 0, "asset_symbol must be HSR");
			FC_ASSERT(!my->_blockchain->get_is_in_sandbox(),"not support sanbox!");
			...
			PublicKeyType  caller_public_key = PublicKeyType(caller_public_key_str);
			Address        caller_address = Address(caller_public_key);
			...
			if (!is_testing)
			{
				get_enough_balances_by_address(caller_public_key_str, asset_limit + fee, balances, required_signatures);
			}
			else
				required_signatures.insert(caller_address);
        ...
        std::vector<hsrcore::blockchain::Asset> Wallet::call_contract_testing(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            FC_ASSERT(is_open(), "Wallet not open!");
            FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            FC_ASSERT(my->is_receive_account(caller), "Invalid account name");
        ...
        std::string Wallet::call_contract_offline(const string caller, const ContractIdType contract, const string method, const string& arguments)
        {
            ...
            if (exception_code > 0)
            {
                hsrcore::blockchain::contract_error con_err(32000, "exception", exception_msg);
                throw con_err;
            }
        ...
        hsrcore::wallet::oScriptEntry Wallet::get_script_entry(const ScriptIdType& script_id)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
        ...
        hsrcore::blockchain::ScriptIdType Wallet::add_script(const fc::path& filename, const string& description/*=string("")*/)
        {
            try{
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
            ...
            const string& hsr_account)
        {
            try {
                ...
                if (hsr_account != "")
                {
                    ...
                    trx.hsr_account = hsr_account;
                    trx.hsr_inport_asset = asset_to_transfer;
                }
                // 	  if (memo_message != "")
                // 	  {
        ...
        hsrcore::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract(
            ...
            bool is_testing)
        {
            try {
                ...
                FC_ASSERT(is_open(), "Wallet not open!");
                FC_ASSERT(is_unlocked(), "Wallet not unlock!");
                FC_ASSERT(amount_to_transfer_symbol == HSR_BLOCKCHAIN_SYMBOL, "Asset symbol should be HSR");
		...
		hsrcore::wallet::WalletTransactionEntry Wallet::transfer_asset_to_contract_without_signature(
			double real_amount_to_transfer,
			const string& amount_to_transfer_symbol,
        ...
        std::vector<hsrcore::blockchain::Asset> Wallet::transfer_asset_to_contract_testing(
            double real_amount_to_transfer,
            const string& amount_to_transfer_symbol,
            const string& from_account_name,
            const Address& to_contract_address,
		...
		std::vector<hsrcore::blockchain::Asset> Wallet::upgrade_contract_testing(
			...
			)
		{
			try
			{
				...
				if (margin_balance_entry->balance != HSR_DEFAULT_CONTRACT_MARGIN)
					FC_CAPTURE_AND_THROW(invalid_margin_amount, (margin_balance_entry->balance));


				Asset margin = Asset(HSR_DEFAULT_CONTRACT_MARGIN);
				Asset fee = get_transaction_fee(0);
				Asset asset_for_exec = my->_blockchain->get_amount(CONTRACT_TESTING_LIMIT_MAX);
			...
			bool is_testing)
        {
            try
			{
				...
				FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not HSR!");
		...
		WalletTransactionEntry Wallet::upgrade_contract_without_signature(
			const Address& contract_id,
			const string& upgrader_public_key_str,
			const string& new_contract_name,
			const string& new_contract_desc,
			const std::string& asset_symbol,
			const double exec_limit,
			bool is_testing)
		{
			try
			{
				...
				PublicKeyType  upgrader_owner_key = PublicKeyType(upgrader_public_key_str);
				Address        upgrader_owner_address = Address(upgrader_owner_key);
				...
				if (upgrader_owner_key != contract_entry->owner)
					FC_CAPTURE_AND_THROW(contract_no_permission, (upgrader_public_key_str));
				...
				if (!is_testing)
				{
					get_enough_balances_by_address(upgrader_public_key_str, required_fees + asset_limit, balances, required_signatures);
		...
		std::vector<hsrcore::blockchain::Asset> Wallet::destroy_contract_testing(
			const Address& contract_id,
			...
			bool is_testing)
        {
            try
			{
                ...
                FC_ASSERT(balance_entry.valid(), "Can not get margin balance!");
                FC_ASSERT(balance_entry->asset_id() == 0, "margin balance asset is not HSR!");
                FC_ASSERT(balance_entry->balance == HSR_DEFAULT_CONTRACT_MARGIN, "invalid margin balance amount");
                ...
                {
                    FC_ASSERT(balance_entry->asset_id() == 0, "contract balance asset is not HSR!");
                }
		...
		WalletTransactionEntry Wallet::destroy_contract_without_signatrue(
			const Address& contract_id,
			const string& destroyer_public_key_str,
			const std::string& asset_symbol,
			double exec_limit,
			bool is_testing)
		{
			try
			{
				...
				PublicKeyType  destroyer_owner_key = PublicKeyType(destroyer_public_key_str);
				Address        destroyer_owner_address = Address(destroyer_owner_key);
				...
				if (destroyer_owner_key != contract_entry->owner)
					FC_CAPTURE_AND_THROW(contract_no_permission, (destroyer_public_key_str));
				...
				if (!is_testing)
				{
					get_enough_balances_by_address(destroyer_public_key_str, required_fees + asset_limit, balances, required_signatures);
				}
				else
					required_signatures.insert(destroyer_owner_address);
            ...
            bool sign)
        {
            try {
                ...
                {
                    required_fees += Asset(HSR_BLOCKCHAIN_REGISTER_ACCOUNT_FEE, 0);
                }
            ...
            bool sign)
        {
            try {
                ...
                FC_ASSERT(HSR_BLOCKCHAIN_MAX_SHARES > max_share_supply_in_internal_units);
        ...
        void Wallet::set_transaction_fee(const Asset& fee)
        {
            try {
                ...
                if (fee.amount < HSR_DEFAULT_TRANSACTION_FEE || fee.asset_id != 0)
        ...
        Asset Wallet::get_transaction_imessage_fee(const std::string & imessage)const
        {
            try
            {
                ...
                Asset require_fee;
                    ...
                    if (HSR_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE >= imessage.size())
                    {
                        return require_fee;
                    }
                    auto min_fee_coe = get_transaction_imessage_fee_coe();
                    require_fee.amount = min_fee_coe * (imessage.size() - HSR_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
        ...
        Asset Wallet::get_transaction_fee(const AssetIdType desired_fee_asset_id)const
        {
            try {
                ...
                Asset xts_fee(HSR_WALLET_DEFAULT_TRANSACTION_FEE, 0);
                try
                {
        ...
        void Wallet::set_last_scanned_block_number_for_hsr(uint32_t block_num)
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                my->_wallet_db.set_property(last_scanned_block_number_for_hsrcore, fc::variant(block_num));
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        uint32_t Wallet::get_last_scanned_block_number_for_hsr()const
        {
            try {
                FC_ASSERT(is_open(), "Wallet not open!");
                try
                {
                    return my->_wallet_db.get_property(last_scanned_block_number_for_hsrcore).as<uint32_t>();
                }
                catch (...)
                {
                }
        ...
        void Wallet::set_transaction_imessage_fee_coe(const ImessageIdType& coe)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (coe < HSR_BLOCKCHAIN_MIN_MESSAGE_FEE_COE)
                    {
        ...
        void Wallet::set_transaction_imessage_soft_max_length(const ImessageLengthIdType& length)
        {
            try
            {
                FC_ASSERT(is_open(), "Wallet not open!");
                    ...
                    if (length < 0 || length > HSR_BLOCKCHAIN_MAX_MESSAGE_SIZE)
                    {
        ...
        void Wallet::set_transaction_expiration(uint32_t secs)
        {
            try {
                ...
                if (secs > HSR_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC)
