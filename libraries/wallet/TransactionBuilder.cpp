...
void  TransactionBuilder::set_wallet_implementation(std::unique_ptr<hsrcore::wallet::detail::WalletImpl>& wimpl)
{
    _wimpl = wimpl.get();
}
...
TransactionBuilder& TransactionBuilder::deposit_asset(const hsrcore::wallet::WalletAccountEntry& payer,
    ...
    const string& hsr_account)
{
    try {
        ...
        if (memo.size() > HSR_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE)
...
TransactionBuilder& TransactionBuilder::deposit_asset_with_escrow(const hsrcore::wallet::WalletAccountEntry& payer,
    const hsrcore::blockchain::AccountEntry& recipient,
    const hsrcore::blockchain::AccountEntry& escrow_agent,
    ...
    const string& hsr_account)
{
    try {
        ...
        // Don't automatically truncate memos as long as users still depend on them via deposit ops rather than mail
        if (memo.size() > HSR_BLOCKCHAIN_MAX_MEMO_SIZE)
            FC_CAPTURE_AND_THROW(memo_too_long, (memo));
    ...
    )
{
    try {
        ...
        if (market_fee >= 0 || market_fee <= HSR_BLOCKCHAIN_MAX_UIA_MARKET_FEE)
            actual_market_fee = HSR_BLOCKCHAIN_MAX_UIA_MARKET_FEE * market_fee;
