...
void WalletImpl::scan_block(uint32_t block_num)
{
	try {
		...
		auto block_signee = block_header.coin_base;
		const auto my_accounts = self->list_my_accounts();
		const auto timestamp = _blockchain->get_genesis_timestamp();
		...
		for (const auto& account : my_accounts)
		{
			if (account.owner_address() == Address(block_signee))
			{
    ...
    bool bNeedcreate)
{
    ...
    // 
        ...
        {
            if (store_entry && transaction.hsr_account != "")
            {
                HSRTrxidBalance hsr_entry;
                ...
                for (const auto item : pretty_trx.ledger_entries)
                {
                    ...
                    {
                        hsr_entry.from_account = item.from_account;
                        break;
                    }
                }
                string strToAccount;
                string strSubAccount;
                self->accountsplit(transaction.hsr_account, strToAccount, strSubAccount);
                auto blockchain_trx = _blockchain->get_transaction(transaction_id);
                bool bDiffDeop = false;
                if (blockchain_trx.valid())
                {
                    ShareType hsr_amount = 0;
                    string preowner;
                    int iLoop = 0;
                ...
                //hsr_entry.from_account = transaction.from_account;
                hsr_entry.create_time = transaction_entry->created_time;
                hsr_entry.hsr_account = transaction.hsr_account;
                hsr_entry.block_num = transaction_entry->block_num;
                transaction_entry->trx.hsr_account = transaction.hsr_account;
                transaction_entry->trx.hsr_inport_asset = transaction.hsr_inport_asset;
                hsr_entry.trx_id = transaction.id();
                    //hsr_entry.asset_trx = transaction.hsr_inport_asset;
                    hsr_entry.asset_trx = Asset(hsr_amount, 0);
                    {
                        writelock wlock(m_mutex_for_hsr);
                _blockchain->transaction_insert_to_hsr_full_entry(strToAccount, hsr_entry);
                //_blockchain->transaction_insert_to_hsr_balance(hsr_entry.hsr_account, hsr_entry);
                    }
                }
            }
        }
...
bool hsrcore::wallet::detail::WalletImpl::scan_withdraw_contract(const WithdrawContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
...
bool hsrcore::wallet::detail::WalletImpl::scan_ondestroycontract_withdraw(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, PublicKeyType key, bool bNeedcreate)
{
	try {
		const auto amount = Asset(op.amount); // Always base asset
...
bool hsrcore::wallet::detail::WalletImpl::scan_ondestroycontract_deposit(const OnDestroyOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try {
...
bool hsrcore::wallet::detail::WalletImpl::scan_deposit_contract(const DepositContractOperation & op, WalletTransactionEntry & trx_rec, Asset & total_fee, bool bHaswithdraw)
{
	try{
		auto entry = LedgerEntry();
    ...
    TrxType trx_search_type)const
{
    try {
        ...
        if (!asset_symbol.empty() && asset_symbol != HSR_BLOCKCHAIN_SYMBOL)
        {
            try
            {
...
PrettyTransaction		Wallet::to_pretty_trx(const hsrcore::blockchain::TransactionEntry& trx_entry, const std::string addr_for_fee) const
{
    ...
    pretty_trx.trx_type = hsrcore::blockchain::TransactionType::normal_transaction;
    ...
    for (const auto & op : trx_entry.trx.operations)
    {
		...
		case withdraw_pay_op_type:
		{
            ...
            pretty_trx.trx_type = hsrcore::blockchain::TransactionType::withdraw_pay_transaction;
		}
		default:
			break;
		}
    ...
    for (const auto& op : trx.operations)
    {
        ...
        case register_account_op_type:
        {
            ...
            if (register_op.delegate_pay_rate > 100)
            {
                ...
                pretty_trx.trx_type = hsrcore::blockchain::TransactionType::register_account_transaction;
            }
            else
            {
                pretty_entry.memo = "register delegate " + account_name_entry->name; // Can't tell if initially registered as a delegate
                pretty_trx.trx_type = hsrcore::blockchain::TransactionType::register_delegate_transaction;
            }
		...
		case update_account_op_type:
		{
            ...
            //所以现在只能简单粗暴得用一个1000HSR来进行划分，只要是大于1000HSR都认为是升级代理
            if (total_fee <= Asset( int64_t(1000) * HSR_BLOCKCHAIN_PRECISION ))
            {
                pretty_entry.memo = "update " + account_name_entry->name;
                pretty_trx.trx_type = hsrcore::blockchain::TransactionType::update_account_transaction;
            }
            else
            {
                pretty_entry.memo = "upgrade " + account_name_entry->name;
                pretty_trx.trx_type = hsrcore::blockchain::TransactionType::upgrade_account_transaction;
            }
...
PrettyContractTransaction		Wallet::to_pretty_contract_trx(const hsrcore::blockchain::TransactionEntry& trx_entry) const
{
    PrettyContractTransaction pretty_trx;
    SignedTransaction result_trx = trx_entry.trx;

    FC_ASSERT((trx_entry.trx.operations.size() > 0) && (trx_entry.trx.operations[0].type == hsrcore::blockchain::OperationTypeEnum::transaction_op_type) , 
        "not result contract transaction");
    SignedTransaction orig_trx = trx_entry.trx.operations[0].as<hsrcore::blockchain::TransactionOperation>().trx;
    ...
    FC_ASSERT((orig_trx.operations.size() > 0) && 
        ((orig_trx.operations[0].type >= fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(register_contract_op_type))
            && (orig_trx.operations[0].type <= fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(transfer_contract_op_type))), "invalid original contract transaction");


    hsrcore::blockchain::OperationTypeEnum contract_op_type = orig_trx.operations[0].type;
    ...
    if (contract_op_type == hsrcore::blockchain::OperationTypeEnum::register_contract_op_type)
    {
        pretty_trx.trx_type = hsrcore::blockchain::TransactionType::register_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        hsrcore::blockchain::RegisterContractOperation register_contract_op = orig_trx.operations[0].as<hsrcore::blockchain::RegisterContractOperation>();
        Address contract_owner = hsrcore::blockchain::Address(register_contract_op.owner);
        ...
        to_contract_ledger_entry.amount = Asset(HSR_DEFAULT_CONTRACT_MARGIN, 0);
        ...
        {
            // 注册的合约成功上链
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(contract_info_op_type))
                register_success = true;
            
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(balances_withdraw_op_type))
            {
        ...
        if (register_success)
        {
            to_contract_ledger_entry.fee = Asset(all_cost - HSR_DEFAULT_CONTRACT_MARGIN, 0);
    ...
    else if (contract_op_type == hsrcore::blockchain::OperationTypeEnum::upgrade_contract_op_type)
    {
        pretty_trx.trx_type = hsrcore::blockchain::TransactionType::upgrade_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        hsrcore::blockchain::UpgradeContractOperation upgrade_contract_op = orig_trx.operations[0].as<hsrcore::blockchain::UpgradeContractOperation>();
        ...
        {
            // 合约成功升级
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(on_upgrade_op_type))
            ...
            // 合约出账(含保证金) 
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(withdraw_contract_op_type))
            {
            ...
            // 用户入账
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(deposit_op_type))
            {
    ...
    else if (contract_op_type == hsrcore::blockchain::OperationTypeEnum::destroy_contract_op_type)
    {
        pretty_trx.trx_type = hsrcore::blockchain::TransactionType::destroy_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        hsrcore::blockchain::DestroyContractOperation destroy_contract_op = orig_trx.operations[0].as<hsrcore::blockchain::DestroyContractOperation>();
        ...
        {
            // 合约成功销毁
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(on_destroy_op_type))
            {
    ...
    else if (contract_op_type == hsrcore::blockchain::OperationTypeEnum::call_contract_op_type)
    {
        pretty_trx.trx_type = hsrcore::blockchain::TransactionType::call_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        hsrcore::blockchain::CallContractOperation call_contract_op = orig_trx.operations[0].as<hsrcore::blockchain::CallContractOperation>();
        Address contract_caller = hsrcore::blockchain::Address(call_contract_op.caller);
        ...
        {
            // 合约成功调用
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(on_call_success_op_type))
                call_success = true;
    ...
    else if (contract_op_type == hsrcore::blockchain::OperationTypeEnum::transfer_contract_op_type)
    {
        pretty_trx.trx_type = hsrcore::blockchain::TransactionType::transfer_contract_transaction;

        PrettyContractLedgerEntry to_contract_ledger_entry;
        vector<PrettyContractLedgerEntry> from_contract_ledger_entries;

        hsrcore::blockchain::TransferContractOperation transfer_contract_op = orig_trx.operations[0].as<hsrcore::blockchain::TransferContractOperation>();
        Address contract_depositer = hsrcore::blockchain::Address(transfer_contract_op.from);
        ...
        {
            // 向合约成功充值
            if (op.type == fc::enum_type<uint8_t, hsrcore::blockchain::OperationTypeEnum>(deposit_contract_op_type))
