...
namespace glua
{
	...
	void GluaContractEngine::execute_contract_api_by_address(std::string contract_id, std::string method, std::string argument, std::string *result_json_string)
	{
		...
		if (_scope->L()->force_stopping == true && _scope->L()->exit_code == LUA_API_INTERNAL_ERROR)
			FC_CAPTURE_AND_THROW(hsrcore::blockchain::lua_executor_internal_error, (""));
		...
		if (exception_code > 0)
		{
			...
			{
				FC_CAPTURE_AND_THROW(hsrcore::blockchain::lua_executor_internal_error, (exception_msg));
			}
		}
	}
	...
	std::shared_ptr<::blockchain::contract_engine::VMModuleByteStream> GluaContractEngine::get_bytestream_from_code(const hsrcore::blockchain::Code& code)
	{
