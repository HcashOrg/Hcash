...
namespace hsrcore {
    namespace blockchain {
        ...
        struct HSRTrxidBalance
        {
            HSRTrxidBalance(){};
            HSRTrxidBalance(string hsr_account, TransactionIdType trx_id, Asset asset_trx, uint32_t
                block_num, string from_account, fc::time_point_sec create_time) :
                hsr_account(hsr_account),
                trx_id(trx_id),
                asset_trx(asset_trx),
                block_num(block_num),
            ...
            friend bool operator > (const HSRTrxidBalance& hsra, const HSRTrxidBalance& hsrb){
                return hsra.block_num > hsrb.block_num;
            }
            friend bool operator < (const HSRTrxidBalance& hsra, const HSRTrxidBalance& hsrb){
                return hsra.block_num < hsrb.block_num;
            }
        ...
        struct HSRBalanceEntry
        {
            std::set<TransactionIdType> hsr_trxid_sort;
            std::multimap<uint32_t, HSRTrxidBalance> hsr_block_sort;
        };
...
FC_REFLECT_ENUM(hsrcore::blockchain::TransactionType,
    (normal_transaction)
    ...
    FC_REFLECT_ENUM(hsrcore::blockchain::ResultTransactionType,
    (origin_transaction)
    (complete_result_transaction)
    (incomplete_result_transaction)
    )

    FC_REFLECT(hsrcore::blockchain::HSRTrxidBalance, (hsr_account)(trx_id)(asset_trx)(block_num)(from_account)(create_time))
    FC_REFLECT(hsrcore::blockchain::HSRBalanceEntry, (hsr_trxid_sort)(hsr_block_sort))
    FC_REFLECT(hsrcore::blockchain::Transaction, (expiration)(reserved)(hsr_account)(hsr_inport_asset)(operations)(result_trx_type)(result_trx_id))
    FC_REFLECT_DERIVED(hsrcore::blockchain::SignedTransaction, (hsrcore::blockchain::Transaction), (signatures))
    FC_REFLECT(hsrcore::blockchain::TransactionLocation, (block_num)(trx_num))
