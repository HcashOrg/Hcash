...
FC_REFLECT_ENUM(hsrcore::blockchain::AccountType,
    (titan_account)
    (public_account)
    (multisig_account)
    )
    FC_REFLECT(hsrcore::blockchain::MultisigMetaInfo,
    (required)
    (owners)
    )
    FC_REFLECT(hsrcore::blockchain::AccountMetaInfo,
    (type)
    (data)
    )
    FC_REFLECT(hsrcore::blockchain::DelegateStats,
    (votes_for)
    (pay_rate)
    (signing_key_history)
    ...
    FC_REFLECT(hsrcore::blockchain::VoteDel,
    (votes)
    (delegate_id)
    )
    FC_REFLECT(hsrcore::blockchain::AccountEntry,
    (id)
    (name)
    (public_data)
    (owner_key)
    ...
    FC_REFLECT(hsrcore::blockchain::DelegatePaySalary, (total_balance)(pay_balance))
