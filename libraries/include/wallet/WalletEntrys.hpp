...
namespace hsrcore {
    namespace wallet {
        ...
        enum PropertyEnum
        {
            ...
            last_scanned_block_number_for_hsrcore
        };
        ...
        struct AccountData : public hsrcore::blockchain::AccountEntry
        {
...
{
    template<>
    struct hash < hsrcore::wallet::ScriptRelationKey >
    {
        size_t operator()(const hsrcore::wallet::ScriptRelationKey& s)const
        {
...
FC_REFLECT_ENUM(hsrcore::wallet::WalletEntryTypeEnum,
...
(last_scanned_block_number_for_hsrcore)
    ...
    FC_REFLECT(hsrcore::wallet::WalletProperty,
    (key)
    (value)
    )

    FC_REFLECT_DERIVED(hsrcore::wallet::AccountData, (hsrcore::blockchain::AccountEntry),
    (is_my_account)
    (approved)
    (is_favorite)
    ...
    FC_REFLECT_DERIVED(hsrcore::wallet::AccountAddressData, (hsrcore::wallet::AccountData),
    (owner_address)
    (active_address)
    )

    FC_REFLECT(hsrcore::wallet::MasterKey,
    (encrypted_key)
    (checksum)
    )

    FC_REFLECT(hsrcore::wallet::KeyData,
    (account_address)
    (public_key)
    ...
    FC_REFLECT_ENUM(hsrcore::wallet::ContactData::ContactTypeEnum,
    (account_name)
    (public_key)
    (address)
    (btc_address)
    ...
    FC_REFLECT(hsrcore::wallet::ContactData,
    (contact_type)
    (data)
    (label)
    (favorite)
    ...
    FC_REFLECT(hsrcore::wallet::TransactionData,
    (entry_id)
    (block_num)
    (is_virtual)
    (is_confirmed)
    ...
    FC_REFLECT(hsrcore::wallet::LedgerEntry,
    (from_account)
    (to_account)
    (amount)
    (memo)
    ...
    FC_REFLECT(hsrcore::wallet::TransactionLedgerEntry,
    (id)
    (block_num)
    (block_id)
    (timestamp)
    ...
    FC_REFLECT_DERIVED(hsrcore::wallet::PrettyTransactionExperimental, (hsrcore::wallet::TransactionLedgerEntry),
    (inputs)
    (outputs)
    (balances)
    (notes)
    ...
    FC_REFLECT(hsrcore::wallet::Setting,
    (name)
    (value)
    )

    FC_REFLECT(hsrcore::wallet::GenericWalletEntry,
    (type)
    (data)
    )

    FC_REFLECT(hsrcore::wallet::ScriptEntry,
    (id)
    (enable)
    (code)
    (description)
    ...
    FC_REFLECT(hsrcore::wallet::ScriptEntryPrintable,
    (id)
    (enable)
    (code_printable)
    (description)
    ...
    FC_REFLECT(hsrcore::wallet::ScriptRelationKey,
    (contract_id)
    (event_type)
    )
...
namespace fc {
    ...
    template<typename T, hsrcore::wallet::WalletEntryTypeEnum N>
    struct get_typename < hsrcore::wallet::WalletEntry<T, N> >
    {
    ...
    template<typename Type, hsrcore::wallet::WalletEntryTypeEnum N>
    struct reflector < hsrcore::wallet::WalletEntry<Type, N> >
    {
        typedef hsrcore::wallet::WalletEntry<Type, N>  type;
        ...
        {
            {
                typedef decltype(((hsrcore::wallet::BaseEntry<N>*)nullptr)->wallet_entry_index) member_type;
                visitor.TEMPLATE operator() < member_type, hsrcore::wallet::BaseEntry<N>, &hsrcore::wallet::BaseEntry<N>::wallet_entry_index > ("index");
            }
