    ...
    namespace wallet {
        ...
        public:
            ...
            */
            ...
            void					set_last_scanned_block_number_for_hsr(uint32_t block_num);
            /**
            ...
            */
            ...
            uint32_t				get_last_scanned_block_number_for_hsr()const;
           ...
           */
            ...
            */
			...
			/**
			* sign a block if this wallet controls the key for the block miner, or throw
			*
			* @param  header  SignedBlockHeader
			*
			* @return void
			*/
			void pow_sign_block(SignedBlockHeader& header)const;

			/**
			* check block work vaildate
			*
			* @param  pblock  FullBlock
			*
			* @return bool
			*/
			bool CheckWork(FullBlock& pblock);


			uint64_t GetStakeWeight();

			/**
			* check stake block work vaildate
			*
			* @param  pblock  FullBlock
			*
			* @return bool
            ...
            *
            * @param wif_key A private key in hsrcore wallet import format
            ...
            * @param amount_to_transfer_symbol the asset to transfer like HSR 
            ...
            * @param hsr_account centralized system address
            *
            * @return WalletTransactionEntry
            */
			...
			hsrcore::wallet::WalletTransactionEntry transfer_asset_to_contract_without_signature(
				double real_amount_to_transfer,
				const string& amount_to_transfer_symbol,
            ...
            std::vector<hsrcore::blockchain::Asset> transfer_asset_to_contract_testing(
                double real_amount_to_transfer,
                const string& amount_to_transfer_symbol,
                const string& from_account_name,
                const Address& to_contract_address,
			...
			WalletTransactionEntry upgrade_contract_without_signature(
				const Address& contract_id,
			...
			std::vector<hsrcore::blockchain::Asset> upgrade_contract_testing(
				const Address& contract_id,
				const string& upgrader_name,
				const string& contract_name,
				const string& contract_desc,
			...
			WalletTransactionEntry destroy_contract_without_signatrue(
				const Address& contract_id,
			...
			std::vector<hsrcore::blockchain::Asset> destroy_contract_testing(
				const Address& contract_id,
				const string& destroyer_name,
				bool sign = true
				);
            ...
            */
            WalletTransactionEntry register_account(
                ...
                hsrcore::blockchain::AccountType new_account_type,
                bool sign
                );
            /**
            ...
            */
            DelegatePaySalary query_delegate_salary(
                const string& delegate_name);
            std::map<std::string, hsrcore::blockchain::DelegatePaySalary> query_delegate_salarys();
            ...
            * @return string
            */
            PrettyTransaction				    to_pretty_trx(const hsrcore::blockchain::TransactionEntry& trx_entry, const std::string addr_for_fee) const;


            PrettyContractTransaction           to_pretty_contract_trx(const hsrcore::blockchain::TransactionEntry& trx_entry) const;
            ...
            */
			...
			WalletTransactionEntry register_contract_without_signature(const string& owner_public_key_str, const fc::path codefile, const string& asset_symbol, double init_limit, bool is_testing = false);

			std::vector<hsrcore::blockchain::Asset> register_contract_testing(const string& owner, const fc::path codefile);
			...
			WalletTransactionEntry call_contract_without_signature(const string caller_public_key_str, const ContractIdType contract, const string method, const string& arguments, const string& asset_symbol, double cost_limit, bool is_testing = false);
			std::vector<hsrcore::blockchain::Asset> call_contract_testing(const string caller, const ContractIdType contract, const string method, const string& arguments);
...
FC_REFLECT_ENUM(hsrcore::wallet::AccountKeyType,
    (owner_key)
    (active_key)
    (signing_key)
    )
