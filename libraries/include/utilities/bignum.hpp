...
public:
    ...
    */
    ...
    unsigned int GetSerializeSize(int nType=0, int nVersion= POW_PROTOCOL_VERSION) const
    {
		return fc::raw::pack_size(getvch(), nType, nVersion);
    }

    template<typename Stream>
    void Serialize(Stream& s, int nType=0, int nVersion= POW_PROTOCOL_VERSION) const
    {
			fc::raw::pack(s, getvch(), nType, nVersion);
    }

    template<typename Stream>
    void Unserialize(Stream& s, int nType=0, int nVersion= POW_PROTOCOL_VERSION)
    {
        std::vector<unsigned char> vch;
		fc::raw::unpack(s, vch, nType, nVersion);
        setvch(vch);
    }
