 ...
 * @author zoowii@hsrcore
 */

#ifndef hsrcore_lua_api_h
#define hsrcore_lua_api_h
...
public:
    ...
    int  contract_state;
    // 合约中storage的类型
    std::map<std::string, hsrcore::blockchain::StorageValueTypes> contract_storage_properties;
...
#define lua_storage_is_table(t) (hsrcore::blockchain::is_any_table_storage_value_type(t)||hsrcore::blockchain::is_any_array_storage_value_type(t))
#define lua_storage_is_array(t) (hsrcore::blockchain::is_any_array_storage_value_type(t))
#define lua_storage_is_hashtable(t) (hsrcore::blockchain::is_any_table_storage_value_type(t))
...
{
    hsrcore::blockchain::StorageValueTypes type;
    union GluaStorageValueUnion value;
    inline GluaStorageValue()
    {
        type = hsrcore::blockchain::StorageValueTypes::storage_value_null;
    }
    inline static GluaStorageValue from_int(int val)
    {
        GluaStorageValue sv;
        sv.type = hsrcore::blockchain::StorageValueTypes::storage_value_int;
    ...
    {
        GluaStorageValue sv;
        sv.type = hsrcore::blockchain::StorageValueTypes::storage_value_string;
        sv.value.string_value = val;
        return sv;
    }

	// 尝试类型转换
	inline void try_parse_type(hsrcore::blockchain::StorageValueTypes new_type)
    {
	    switch(new_type)
	    {
    ...
    {
	    if(type == hsrcore::blockchain::StorageValueTypes::storage_value_number)
	    {
			value.int_value = (lua_Integer)value.number_value;
			type = hsrcore::blockchain::StorageValueTypes::storage_value_int;
	    }
    }
    ...
    {
		if (type == hsrcore::blockchain::StorageValueTypes::storage_value_int)
		{
			value.number_value = (lua_Number)value.int_value;
			type = hsrcore::blockchain::StorageValueTypes::storage_value_number;
		}
    }

    inline static bool is_same_base_type_with_type_parse(hsrcore::blockchain::StorageValueTypes type1, hsrcore::blockchain::StorageValueTypes type2)
    {
        return type1 == type2;
    }
    inline bool equals(GluaStorageValue &other)
    {
        ...
        case hsrcore::blockchain::StorageValueTypes::storage_value_null:
            return true;
        default: {
            if (hsrcore::blockchain::is_any_table_storage_value_type(type)
                || hsrcore::blockchain::is_any_array_storage_value_type(type))
            {
        ...
        namespace api {
          ...
          public:
            ...
            */
            ...
            /**
            * get contract info stored before from hsrcore api
            ...
            */
            ...
            virtual std::shared_ptr<GluaModuleByteStream> get_bytestream_from_code(lua_State *L, const hsrcore::blockchain::Code& code) = 0;
            /**
            ...
            */
            ...
            /**
             * get contract address/id from hsrcore by contract name
             */
            ...
            /**
             * register new storage name of contract to hsrcore
             */
            virtual bool register_storage(lua_State *L, const char *contract_name, const char *name) = 0;

            virtual GluaStorageValue get_storage_value_from_hsrcore(lua_State *L, const char *contract_name, std::string name) = 0;

            virtual GluaStorageValue get_storage_value_from_hsrcore_by_address(lua_State *L, const char *contract_address, std::string name) = 0;

            /**
             * after lua merge storage changes in lua_State, use the function to store the merged changes of storage to hsrcore
             */
            virtual bool commit_storage_changes_to_hsrcore(lua_State *L, AllContractsChangesMap &changes) = 0;
