...
namespace hsrcore {
    namespace rpc {
        ...
        /**
        *  @class rpc_server
        *  @brief provides a json-rpc interface to the hsrcore client
        */
        class RpcServer
        {
        public:
            RpcServer(hsrcore::client::Client* client);
            virtual ~RpcServer();

            bool        configure_rpc(const hsrcore::client::RpcServerConfig& cfg);
            bool        configure_http(const hsrcore::client::RpcServerConfig& cfg);
            bool        configure_encrypted_rpc(const hsrcore::client::RpcServerConfig& cfg);
            ...
            const hsrcore::api::MethodData& get_method_data(const std::string& method_name);
            std::vector<hsrcore::api::MethodData> get_all_method_data() const;
        ...
        protected:
            friend class hsrcore::rpc::detail::RpcServerImpl;

            void validate_method_data(hsrcore::api::MethodData method);
            void register_method(hsrcore::api::MethodData method);
