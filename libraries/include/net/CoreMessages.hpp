...
namespace hsrcore {
    namespace net {
        ...
        const uint32_t core_protocol_version = HSR_NET_PROTOCOL_VERSION;
...
FC_REFLECT_ENUM(hsrcore::net::CoreMessageTypeEnum,
    (trx_message_type)
    (block_message_type)
    ...
    FC_REFLECT(hsrcore::net::ItemId, (item_type)
    (item_hash))
    FC_REFLECT(hsrcore::net::ItemIdsInventoryMessage, (item_type)
    (item_hashes_available))
    FC_REFLECT(hsrcore::net::BlockchainItemIdsInventoryMessage, (total_remaining_item_count)
    (item_type)
    (item_hashes_available))
    FC_REFLECT(hsrcore::net::FetchBlockchainItemIdsMessage, (item_type)
    (blockchain_synopsis))
    FC_REFLECT(hsrcore::net::FetchItemsMessage, (item_type)
    (items_to_fetch))
    FC_REFLECT(hsrcore::net::ItemNotAvailableMessage, (requested_item))
    FC_REFLECT(hsrcore::net::HelloMessage, (user_agent)
    ...
    FC_REFLECT_EMPTY(hsrcore::net::ConnectionAcceptedMessage)
    FC_REFLECT_ENUM(hsrcore::net::RejectionReasonCode, (unspecified)
    (different_chain)
    (already_connected)
    (connected_to_self)
    (not_accepting_connections)
    ...
    (invalid_hello_message)
    (client_too_old))
    FC_REFLECT(hsrcore::net::ConnectionRejectedMessage, (user_agent)
    (core_protocol_version)
    (remote_endpoint)
    (reason_code)
    (reason_string))
    FC_REFLECT_EMPTY(hsrcore::net::AddressRequestMessage)
    FC_REFLECT(hsrcore::net::AddressInfo, (remote_endpoint)
    (last_seen_time)
    (latency)
    (node_id)
    (direction)
    (firewalled))
    FC_REFLECT(hsrcore::net::AddressMessage, (addresses))
    FC_REFLECT(hsrcore::net::ClosingConnectionMessage, (reason_for_closing)
    (closing_due_to_error)
    (error))
    FC_REFLECT_ENUM(hsrcore::net::PeerConnectionDirection, (unknown)
    (inbound)
    (outbound))
    FC_REFLECT_ENUM(hsrcore::net::FirewalledState, (unknown)
    (firewalled)
    (not_firewalled))

    FC_REFLECT(hsrcore::net::CurrentTimeRequestMessage, (request_sent_time))
    FC_REFLECT(hsrcore::net::CurrentTimeReplyMessage, (request_sent_time)
    (request_received_time)
    (reply_transmitted_time))
    FC_REFLECT_ENUM(hsrcore::net::FirewallCheckResult, (unable_to_check)
    (unable_to_connect)
    (connection_successful))
    FC_REFLECT(hsrcore::net::CheckFirewallMessage, (node_id)(endpoint_to_check))
    FC_REFLECT(hsrcore::net::CheckFirewallReplyMessage, (node_id)(endpoint_checked)(result))
    FC_REFLECT_EMPTY(hsrcore::net::GetCurrentConnectionsRequestMessage)
    FC_REFLECT(hsrcore::net::CurrentConnectionData, (connection_duration)
    (remote_endpoint)
    (node_id)
    (clock_offset)
    (round_trip_delay)
    ...
    (firewalled)
    (user_data))
    FC_REFLECT(hsrcore::net::GetCurrentConnectionsReplyMessage, (upload_rate_one_minute)
...
{
    template<>
    struct hash < hsrcore::net::ItemId >
    {
        size_t operator()(const hsrcore::net::ItemId& item_to_hash) const
        {
