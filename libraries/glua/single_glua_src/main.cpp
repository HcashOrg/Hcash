...
static int addreturn(lua_State *L) {
	...
	auto changed_line = hsrcore::lua::lib::pre_modify_lua_source_code_string(L, "", false, code, error, NULL, true, false, NULL, throw_exception, true);
...
static int handle_script(lua_State *L, char **argv, bool is_contract=false) {
	...
	else
	{
		...
		if(!hsrcore::lua::lib::compilefile_to_file(fname, out_fname.c_str(), error, use_type_check_compile))
		{
		...
		return hsrcore::lua::lib::run_compiledfile(L, out_fname.c_str()) ? LUA_OK : LUA_ERRRUN;
	}
}
...
static int run_decompile_script(lua_State *L, char **argv, int script)
{
	...
	auto result = hsrcore::lua::lib::decompile_bytecode_file_to_source_code(L, fname, filepath.stem().string(), error);
...
static int run_compile_script(lua_State *L, char **argv, int script)
{
  ...
  char error[LUA_VM_EXCEPTION_STRNG_MAX_LENGTH] = "\0";
  auto result = hsrcore::lua::lib::compilefile_to_file(fname, out_path.c_str(), error, true);
  if (strlen(error) > 0)
  {
    perror(error);
  }
...
static int run_disassemble_script(lua_State *L, char **argv, int script)
{
	...
	boost::filesystem::path filepath(fname);
	char error[LUA_VM_EXCEPTION_STRNG_MAX_LENGTH] = "\0";
	auto result = hsrcore::lua::lib::disassemble_bytecode_file(L, fname, filepath.stem().string(), error);
