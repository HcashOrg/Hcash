	...
	namespace lua {
		namespace api {
			...
			*/
			...
			std::shared_ptr<GluaModuleByteStream> DemoGluaChainApi::get_bytestream_from_code(lua_State *L, const hsrcore::blockchain::Code& code)
			{
				return nullptr;
			}
			...
			{
				std::string filename = std::string("hsrcore_lua_modules") + glua::util::file_separator_str() + "hsrcore_contract_" + name;
			...
			*/
			...
			GluaStorageValue DemoGluaChainApi::get_storage_value_from_hsrcore(lua_State *L, const char *contract_name, std::string name)
			{
              // fetch storage value from hsrcore
			...
			GluaStorageValue DemoGluaChainApi::get_storage_value_from_hsrcore_by_address(lua_State *L, const char *contract_address, std::string name)
			{
				// fetch storage value from hsrcore
                if (_demo_chain_storage_buffer.find(L) == _demo_chain_storage_buffer.end()) {
                  if (_demo_chain_storage_buffer.size() > 5)
			...
			bool DemoGluaChainApi::commit_storage_changes_to_hsrcore(lua_State *L, AllContractsChangesMap &changes)
			{
				// printf("commited storage changes to hsrcore\n");
                if (_demo_chain_storage_buffer.find(L) == _demo_chain_storage_buffer.end()) {
                  if (_demo_chain_storage_buffer.size() > 5)
                  {
                    _demo_chain_storage_buffer.clear();
