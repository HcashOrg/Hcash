...
// FIXME: duplicate code in hsrcore_lua_lib.cpp
...
// FIXME: end duplicate code in hsrcore_lua_lib.cpp
...
newframe:  /* reentry point when frame changes (call/return) */
    ...
    int insts_limit = hsrcore::lua::lib::get_lua_state_value(L, INSTRUCTIONS_LIMIT_LUA_STATE_MAP_KEY).int_value;
    ...
    if (nullptr == stopped_pointer)
    {
        ...
        hsrcore::lua::lib::resume_lua_state_running(L);
        stopped_pointer = hsrcore::lua::lib::get_lua_state_value(L, LUA_STATE_STOP_TO_RUN_IN_LVM_STATE_MAP_KEY).int_pointer_value;
    }
    int has_insts_limit = insts_limit > 0 ? 1 : 0;
    int *insts_executed_count = hsrcore::lua::lib::get_lua_state_value(L, INSTRUCTIONS_EXECUTED_COUNT_LUA_STATE_MAP_KEY).int_pointer_value;
    if (nullptr == insts_executed_count)
    {
        ...
        *insts_executed_count = 0;
        ...
        hsrcore::lua::lib::set_lua_state_value(L, INSTRUCTIONS_EXECUTED_COUNT_LUA_STATE_MAP_KEY, lua_state_value_of_exected_count, LUA_STATE_VALUE_INT_POINTER);
    }
    ...
    for (;;) {
		...
		if(L->bytecode_debugger_opened)
		{
			...
			hsrcore::lua::core::LuaDebugFileInfo *ldf = nullptr;
			if (proto_source_ldf_filename[0] == '@')
			{
				...
				if (ldf_file)
				{
					auto ldf_deserialized = hsrcore::lua::core::LuaDebugFileInfo::deserialize_from_file(ldf_file);
					ldf = new hsrcore::lua::core::LuaDebugFileInfo();
