...
{"Stream", "() => Stream"},
			...
			{
				const char *system_asset_symbol = hsrcore::lua::api::global_glua_chain_api->get_system_asset_symbol(L);
            ...
            static int get_contract_balance_amount(lua_State *L)
            {
                ...
                auto result = hsrcore::lua::api::global_glua_chain_api->get_contract_balance_amount(L, contract_address, assert_symbol);
            ...
            {
                auto time = hsrcore::lua::api::global_glua_chain_api->get_chain_now(L);
            ...
            {
                auto rand = hsrcore::lua::api::global_glua_chain_api->get_chain_random(L);
                lua_pushinteger(L, rand);
                return 1;
            }
            static int get_transaction_id(lua_State *L)
            {
                std::string tid = hsrcore::lua::api::global_glua_chain_api->get_transaction_id(L);
                lua_pushstring(L, tid.c_str());
                return 1;
            }
            static int get_transaction_fee(lua_State *L)
            {
                int64_t res = hsrcore::lua::api::global_glua_chain_api->get_transaction_fee(L);
                lua_pushinteger(L, res);
                return 1;
            }
            static int get_header_block_num(lua_State *L)
            {
                auto result = hsrcore::lua::api::global_glua_chain_api->get_header_block_num(L);
                lua_pushinteger(L, result);
                return 1;
            }
            ...
            static int get_waited_block_random(lua_State *L)
            {
                ...
                auto result = hsrcore::lua::api::global_glua_chain_api->get_waited(L, (uint32_t)num);
                lua_pushinteger(L, result);
                return 1;
            }

            static int emit_hsrcore_event(lua_State *L)
            {
				...
				if (!contract_id || strlen(contract_id) < 1)
					return 0;
                hsrcore::lua::api::global_glua_chain_api->emit(L, contract_id, event_name, event_param);
                return 0;
            }
			...
			static int glua_core_lib_Stream_size(lua_State *L)
            {
				...
				if(hsrcore::lua::api::global_glua_chain_api->is_object_in_pool(L, (intptr_t) stream,
			...
			static int glua_core_lib_Stream(lua_State *L)
            {
				...
				hsrcore::lua::api::global_glua_chain_api->register_object_in_pool(L, (intptr_t) stream, GluaOutsideObjectTypes::OUTSIDE_STREAM_STORAGE_TYPE);
			...
			*/
			...
			//    hsrcore.error('only string can be storage key')
			//	  return nil
			//	  end
			//	  return hsrcore.get_storage(s.contract, key)
			//	  end
			static int glua_core_lib_storage_metatable_index(lua_State *L)
            {
				...
				auto ret_count = glua::lib::hsrcorelib_get_storage_impl(L, contract_id, key); // top=ret_count + 4
				if(ret_count>0)
				{
					auto tmp_global_key = "_glua_core_lib_hsrcore_get_storage_tmp_value";
			...
			//	return nil
			//	end
			//	return hsrcore.set_storage(s.contract, key, val)
			//	end
			static int glua_core_lib_storage_metatable_new_index(lua_State *L)
            {
                ...
                return glua::lib::hsrcorelib_set_storage_impl(L, contract_id, key, 3);

                /*
				int old_top = lua_gettop(L); // 3
				lua_getfield(L, 1, "contract"); // top=4
				lua_pushcfunction(L, glua::lib::hsrcorelib_set_storage); // storage, key, val, contract, set_storage
        ...
        return hashes[i-int_key_size], t[hashes[i-int_key_size]] 
			...
			*/
            ...
            void close_lua_state(lua_State *L)
            {
				...
				hsrcore::lua::api::global_glua_chain_api->release_objects_in_pool(L);
                LStatesMap *states_map = get_lua_states_value_hashmap();
            ...
            */
            ...
            void set_lua_state_value(lua_State *L, const char *key, GluaStateValue value, enum GluaStateValueType type)
            {
                ...
                if (node_v.type == LUA_STATE_VALUE_STRING)
                    node_v.value.string_value = hsrcore::lua::lib::malloc_and_copy_string(L, value.string_value);
                std::string key_str(key);
                map->erase(key_str);
...
#define MYK(x)		(-1-(x))
	...
	storage: S
				...
				bool throw_exception, bool in_repl, bool is_contract)
            {
				...
				if (use_type_check)
				{
					...
					if(is_contract)
					{
						...
						{
							if(!hsrcore::lua::api::global_glua_chain_api->check_contract_exist(L, p.first.c_str()))
							{
					...
					if(is_importing_contract)
					{
						...
						if(getOpMode(o) == OP_LOADK)
						{
							...
							if (idx_in_kst >= 0 && idx_in_kst < proto->sizek)
							{
								...
								if (contract_name && !hsrcore::lua::api::global_glua_chain_api->check_contract_exist(L, contract_name))
								{
					...
					else if(is_importing_contract_address)
					{
						...
						if (getOpMode(o) == OP_LOADK)
						{
							...
							if (idx_in_kst >= 0 && idx_in_kst < proto->sizek)
							{
								...
								if (contract_address && !hsrcore::lua::api::global_glua_chain_api->check_contract_exist_by_address(L, contract_address))
								{
                ...
                char *error, GluaStatePreProcessorFunction *preprocessor, bool use_type_check)
            {
                ...
                if (hsrcore::lua::api::global_glua_chain_api->has_exception(L))
                    hsrcore::lua::api::global_glua_chain_api->clear_exceptions(L);
			...
			std::stack<std::string> *get_using_contract_id_stack(lua_State *L, bool init_if_not_exist)
            {
				...
				auto contract_id_stack_value_in_state_map = hsrcore::lua::lib::get_lua_state_value(L, GLUA_CONTRACT_API_CALL_STACK_STATE_MAP_KEY);
				if (!contract_id_stack_value_in_state_map.pointer_value)
				{
					...
					hsrcore::lua::lib::set_lua_state_value(L, GLUA_CONTRACT_API_CALL_STACK_STATE_MAP_KEY, contract_id_stack_value_in_state_map, GluaStateValueType::LUA_STATE_VALUE_POINTER);
				}
				else
				...
				const char *api_name, const char *arg1, std::string *result_json_string)
            {
                ...
                hsrcore::lua::api::global_glua_chain_api->get_contract_address_by_name(L, contract_name, contract_address, &address_size);
                if (address_size > 0)
                {
                    GluaStateValue value;
            ...
            {
				auto starting_contract_address_node = hsrcore::lua::lib::get_lua_state_value_node(L, STARTING_CONTRACT_ADDRESS);
