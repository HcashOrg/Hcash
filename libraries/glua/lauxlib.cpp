...
static const char *progname = "hsrcore_lua";
...
static int contract_api_wrapper_func(lua_State *L)
{
	...
	// contract_id压栈
	auto contract_id_stack = hsrcore::lua::lib::get_using_contract_id_stack(L, true);
...
{
    int *stopped_pointer = hsrcore::lua::lib::get_lua_state_value(L, LUA_STATE_STOP_TO_RUN_IN_LVM_STATE_MAP_KEY).int_pointer_value;
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        ...
        global_glua_chain_api->get_contract_address_by_name(L, hsrcore::lua::lib::unwrap_any_contract_name(name.c_str()).c_str(), contract_id, &contract_id_size);
...
void lua_fill_contract_info_for_use(lua_State *L)
{
    ...
    lua_getglobal(L, "hsrcore"); // {}, hsrcore
    lua_pushvalue(L, -2); // {}, hsrcore, {}
    lua_getfield(L, -2, "storage_mt"); // {}, hsrcore, {}, storage_mt
    lua_setmetatable(L, -2); // {}, hsrcore, {}   设置storage的metatable为storage_mt
    lua_pop(L, 3);
}
...
static std::string get_contract_name_using_in_lua(std::string namestr)
{
    ...
    return use_self_name ? CURRENT_CONTRACT_NAME : hsrcore::lua::lib::unwrap_any_contract_name(namestr.c_str());
}
...
static std::string get_contract_id_using_in_lua(lua_State *L, std::string namestr, bool is_pointer, bool is_stream)
{
    ...
    else if (!is_pointer && !is_stream)
    {
        ...
        global_glua_chain_api->get_contract_address_by_name(L, hsrcore::lua::lib::unwrap_any_contract_name(namestr.c_str()).c_str(), address, &address_len);
        address[CONTRACT_ID_MAX_LENGTH-1] = '\0';
        return address;
    }
    else
    {
		auto name = hsrcore::lua::lib::unwrap_any_contract_name(namestr.c_str());
		char address[CONTRACT_ID_MAX_LENGTH];
		memset(address, 0x0, sizeof(char) * CONTRACT_ID_MAX_LENGTH);
		size_t address_len = 0;
        global_glua_chain_api->get_contract_address_by_name(L, hsrcore::lua::lib::unwrap_any_contract_name(name.c_str()).c_str(), address, &address_len);
		address[CONTRACT_ID_MAX_LENGTH - 1] = '\0';
		return address;
...
int luaL_import_contract_module_from_address(lua_State *L)
{
    ...
    std::string name_str;
    name_str = std::string(ADDRESS_CONTRACT_PREFIX) + hsrcore::lua::lib::unwrap_any_contract_name(contract_id);
    name = name_str.c_str();
    auto unwrap_name = hsrcore::lua::lib::unwrap_any_contract_name(contract_id);
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        ...
        {
			for(const auto &special_api_name : hsrcore::lua::lib::contract_special_api_names)
			{
				lua_pushnil(L);
 ...
 * restrain name must be pure file name, with no '.lua' extension. And can load bytestream from hsrcore api.
...
int luaL_import_contract_module(lua_State *L)
{
    ...
    {
        name_str = hsrcore::lua::lib::wrap_contract_name(origin_contract_name);
        name = name_str.c_str();
    }
    auto unwrap_name = hsrcore::lua::lib::unwrap_any_contract_name(origin_contract_name);
    const char *filename = name;
    lua_settop(L, 1);  /* _LOADED table will be at index 2 */
    lua_getfield(L, LUA_REGISTRYINDEX, "_LOADED");
    lua_getfield(L, 2, filename);  /* _LOADED[name] */
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        ...
        if (!is_starting_contract)
        {
			for(const auto &api_name : hsrcore::lua::lib::contract_special_api_names)
			{
				lua_pushnil(L);
...
)
{
    ...
    std::string wrapper_contract_name_str = hsrcore::lua::lib::wrap_contract_name(contract_name);
    std::string unwrapper_name = hsrcore::lua::lib::unwrap_any_contract_name(contract_name);
    if (!is_address)
    ...
    if (lua_isfunction(L, -1))
    {
		...
		if (glua::util::vector_contains(hsrcore::lua::lib::contract_int_argument_special_api_names, api_name_str))
		{
...
{
	auto contract_address = hsrcore::lua::lib::malloc_managed_string(L, CONTRACT_ID_MAX_LENGTH + 1);
	...
	if (address_size > 0)
	{
		...
		hsrcore::lua::lib::set_lua_state_value(L, STARTING_CONTRACT_ADDRESS, value, LUA_STATE_VALUE_STRING);
	}
	...
	struct LuaCompileFilePreloadResult *result, bool use_type_check = false, GluaModuleByteStreamP stream = nullptr, bool is_contract = false)
{
	...
	auto code = hsrcore::lua::lib::pre_modify_lua_source_code(L, filename, error, nullptr, use_type_check, stream, is_contract);
	if (global_glua_chain_api->has_exception(L))
    {
    ...
    if (status != 0)
    {
		...
		if (lf.f) fclose(lf.f);
        const char *error_str = "parse hsrcore lua code error";
...
{
    auto lua_table_map_list_p = hsrcore::lua::lib::get_lua_state_value(L, LUA_TABLE_MAP_LIST_STATE_MAP_KEY).pointer_value;
    if (nullptr == lua_table_map_list_p)
    {
        ...
        hsrcore::lua::lib::set_lua_state_value(L, LUA_TABLE_MAP_LIST_STATE_MAP_KEY, value, LUA_STATE_VALUE_POINTER);
    }
...
struct GluaStorageValue lua_type_to_storage_value_type_with_nested(lua_State *L, int index, size_t len, std::list<const void *> &jsons, size_t recur_depth)
{
    ...
    if (index > lua_gettop(L))
    {
        storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_not_support;
        storage_value.value.int_value = 0;
        return storage_value;
    }
	if(recur_depth>LUA_MAP_TRAVERSER_MAX_DEPTH)
	{
		storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_null;
		storage_value.value.int_value = 0;
		return storage_value;
	}
    ...
    case LUA_TNIL:
        ...
        return storage_value;
    case LUA_TBOOLEAN:
        storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_bool;
        storage_value.value.bool_value = BOOL_VAL(lua_toboolean(L, index));
        return storage_value;
    case LUA_TNUMBER:
        if (lua_isinteger(L, index))
        {
            storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_int;
            storage_value.value.int_value = (lua_Integer)lua_tointeger(L, index);
            return storage_value;
        }
        storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_number;
        storage_value.value.number_value = lua_tonumber(L, index);
        return storage_value;
    case LUA_TSTRING:
        storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_string;
    ...
    case LUA_TTABLE:
		...
		// FIXME: 根据子项类型修改
        storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_unknown_table;
        if (len > 0)
            storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_unknown_array;
		...
		// 如果是Stream，转成stream类型
	{
		...
		{
			storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_stream;
			storage_value.value.userdata_value = addr;
		}
		else
		{
			storage_value.type = hsrcore::blockchain::StorageValueTypes::storage_value_userdata;
...
bool lua_table_to_map_traverser_with_nested(lua_State *L, void *ud, size_t len, std::list<const void*> &jsons, size_t recur_depth)
{
    ...
    if (lua_gettop(L) < 2)
        return false;
    if (!lua_isstring(L, -2) && !lua_isinteger(L, -1)) // now only support integer and string as table key, when using by hsrcore
        return false;
    std::string key;
    ...
    {
        value.type = hsrcore::blockchain::StorageValueTypes::storage_value_string;
...
static void luatablemap_to_json_stream(GluaTableMapP map, std::stringstream &ss, bool is_array=false)
{
    ...
    for (auto it = map->begin(); it != map->end(); ++it)
    {
        ...
        switch (value.type)
        {
		case hsrcore::blockchain::StorageValueTypes::storage_value_null:
            ss << "null";
            break;
		case hsrcore::blockchain::StorageValueTypes::storage_value_bool:
            ss << (value.value.bool_value ? "true" : "false");
            break;
		case hsrcore::blockchain::StorageValueTypes::storage_value_int:
            ss << value.value.int_value;
            break;
		case hsrcore::blockchain::StorageValueTypes::storage_value_number:
            ss << value.value.number_value;
            break;
		case hsrcore::blockchain::StorageValueTypes::storage_value_string:
        {
            auto str=std::string(value.value.string_value);
            ss << "\"" << glua::util::escape_string(str) << "\"";
            break;
        }
		case hsrcore::blockchain::StorageValueTypes::storage_value_userdata:
            ss << "\"userdata\"";
		default: 
		{
