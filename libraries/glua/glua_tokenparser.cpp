...
namespace glua
{
    namespace parser
    {
        ...
        static inline bool is_hsrha(int c)
        {
            return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z');
        }

        static inline bool is_big_hsrha(int c)
        {
            return 'A' <= c && c <= 'Z';
        }

        static inline bool is_little_hsrha(int c)
        {
            return 'a' <= c && c <= 'z';
        }
        ...
        {
            return is_digit(c) ? (c - '0') : (is_big_hsrha(c) ? (c - 'A' + 10) : (c - 'a' + 10));
        }
            ...
            while (_parsing_pos < code.length())
            {
                ...
                default: {
                  ...
                  if (c < 0)
                  {
                    ...
                    if (is_hsrha(c) || c == '_') {  // identifier or reserved word?
                        do {
                            _save_to_buff_and_next(_current_char(code));
                        } while (is_hsrha(_current_char(code)) || _current_char(code) == '_' || is_digit(_current_char(code)));
