...
namespace hsrcore {
    namespace blockchain {
        ...
        namespace detail
        {
            void ChainDatabaseImpl::revalidate_pending()
            {
                ...
                while (itr.valid())
                {
                    ...
                    try
                    {
                        ...
                        count++;
                        if (count > HSR_BLOCKCHAIN_REVALIDATE_MAX_TRX_COUNT)
                            break;
            ...
            bool ChainDatabaseImpl::replay_required(const fc::path& data_dir)
            {
                try {
                    ...
                    return !entry.valid() || entry->value.as_uint64() != HSR_BLOCKCHAIN_DATABASE_VERSION;
                } FC_CAPTURE_AND_RETHROW((data_dir))
            }
            ...
            void ChainDatabaseImpl::open_database(const fc::path& data_dir)
            {
                try {
                    ...
                    _hsr_input_balance_entry.open(data_dir / "index/_hsr_input_balance_entry");
                    _hsr_full_entry.open(data_dir / "index/_hsr_full_entry");
					...
					_block_id_to_modify.open(data_dir / "index/_block_id_to_modify");
            ...
            DigestType ChainDatabaseImpl::initialize_genesis(const optional<path>& genesis_file, const bool statistics_enabled)
            {
                try {
                    ...
                    // Check genesis state
                    FC_ASSERT(config.delegates.size() >= HSR_BLOCKCHAIN_NUM_DELEGATES,
                        "genesis.json does not contain enough initial delegates!",
                        ("required", HSR_BLOCKCHAIN_NUM_DELEGATES)("provided", config.delegates.size()));
                    ...
                    {
                        //static const vector<string> hsr_prefixes{ "HSR", "KEY", "DVS", "XTS" };
                        try
                        {
                        ...
                        {
                            //for( const string& prefix : hsr_prefixes )
                            //{
                            // if( raw_address.find( prefix ) == 0 )
                            //  {
                            //  return address( HSR_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                            // }
                            //}
                        }
                    ...
                    for (const auto& genesis_balance : config.initial_balances)
                    {
						...
						initial_balance.last_update_transaction_id = fc::ripemd160();
                    ...
                    base_asset.symbol = HSR_BLOCKCHAIN_SYMBOL;
                    base_asset.name = HSR_BLOCKCHAIN_NAME;
                    base_asset.description = HSR_BLOCKCHAIN_DESCRIPTION;
                    base_asset.public_data = variant("");
                    base_asset.issuer_account_id = 0;
                    base_asset.precision = HSR_BLOCKCHAIN_PRECISION;
                    base_asset.registration_date = timestamp;
                    base_asset.last_update = timestamp;
                    base_asset.current_share_supply = total_base_supply;
                    base_asset.maximum_share_supply = HSR_BLOCKCHAIN_MAX_SHARES;
                    ...
                    for (const GenesisAsset& asset : config.market_assets)
                    {
                        ...
                        rec.maximum_share_supply = HSR_BLOCKCHAIN_MAX_SHARES;
				...
				oBlockEntry& entry,bool is_coin_stake)const
			{
				...
				const ShareType max_collected_fees = base_asset_entry->collected_fees;
				const ShareType accepted_collected_fees = max_collected_fees;

				FC_ASSERT(max_collected_fees >= 0 && accepted_collected_fees >= 0, "Invalid collected fee");
				...
				{
					cur_entry->deposit_date = pending_state->now();
				}
				else
				{
					...
					fc::uint128 avg = (old_sec_since_epoch * cur_entry->balance) + (new_sec_since_epoch * accepted_paycheck);
					avg /= (fc::uint128(cur_entry->balance) + fc::uint128(accepted_paycheck));

					cur_entry->deposit_date = time_point_sec(avg.to_integer());
				}
				fc::safe<ShareType> temp = cur_entry->balance;
				temp += accepted_paycheck;
				cur_entry->balance = temp.value;
				cur_entry->last_update = pending_state->now();
				cur_entry->last_update_transaction_id = block_id;
				pending_state->store_balance_entry(*cur_entry);
            ...
            {
                try {
                    // if( pending_state->get_head_block_num() < HSR_V0_6_0_FORK_BLOCK_NUM )
                    ...
                    const ShareType max_new_shares = self->get_max_delegate_pay_issued_per_block(0);
                    const ShareType accepted_new_shares = (max_new_shares * pay_rate_percent) / 100;
                    base_asset_entry->current_share_supply += accepted_new_shares;

                    static const uint32_t blocks_per_two_weeks = 14 * HSR_BLOCKCHAIN_BLOCKS_PER_DAY;
                ...
                const PendingChainStatePtr& pending_state)
            {
                try {
                    ...
                    if (block_num > HSR_BLOCKCHAIN_MAX_UNDO_HISTORY)
                    {
                        const uint32_t old_block_num = block_num - HSR_BLOCKCHAIN_MAX_UNDO_HISTORY;
			...
			void ChainDatabaseImpl::verify_header(const DigestBlock& block_digest, const PublicKeyType& block_signee)const
			{
				try {
					...
					{
						if (!self->CheckProofOfStake((SignedBlockHeader&)block_digest,1))
						{
							FC_CAPTURE_AND_THROW(invalid_block_pow_hash);
						}
					}
					else
					{
						if (block_digest.GetPoWHash() > CBigNum().SetCompact(block_digest.nbits).getuint256())
							FC_CAPTURE_AND_THROW(invalid_block_pow_hash);
					}
            ...
            /**
             *  A block should be produced every HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
                ...
                const PendingChainStatePtr& pending_state)const
            {
                try {
                    ...
                    //if (head_block.block_num > 0) block_timestamp = head_block.timestamp + HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    ...
                    //    block_timestamp += HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
                    ...
                    //if (required_confirmations > HSR_BLOCKCHAIN_NUM_DELEGATES * 3)
                    //    required_confirmations = 3 * HSR_BLOCKCHAIN_NUM_DELEGATES;
			...
			void ChainDatabaseImpl::update_block_modify(const SignedBlockHeader & block_header)const
			{
				try
				{
					fc::ripemd160::encoder enc;
					fc::ripemd160 last_modify;
					auto temp_modify = self->get_head_block_modify(block_header.previous);
					if (temp_modify.valid())
					{
						last_modify = *temp_modify;
					}
					else if (block_header.block_num  == 1)
					{
						last_modify = fc::ripemd160();
					}
					else
					{
						FC_THROW("error modify!");
					}
					...
					if (block_header.is_coinstake)
					{
						...
						{
							fc::raw::pack(enc, balance_entry->last_update_transaction_id);
						}
					}
					else
					{
						fc::raw::pack(enc, block_header.id());
					}
					self->store_block_modify(block_header.id(), enc.result());
				}
            ...
            {
                try {//todo remove????
                    // if( pending_state->get_head_block_num() < HSR_V0_7_0_FORK_BLOCK_NUM )
                    // return update_active_delegate_list_v1( block_num, pending_state );

                    if (block_num % HSR_BLOCKCHAIN_NUM_DELEGATES != 0)
                        return;
				...
				try {
					...
					try
					{
						...
						**/
						...
						//remove fee protect
						//if (hsrcore::client::g_client->get_wallet() != nullptr&&hsrcore::client::g_client->get_wallet()->is_open())
						//{
						//	if (!hsrcore::client::g_client->get_wallet()->get_my_delegates(hsrcore::wallet::enabled_delegate_status).empty())
						//	{
						//		auto fee = hsrcore::client::g_client->get_delegate_config().transaction_min_fee;
						//		_relay_fee = fee > HSR_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee : HSR_BLOCKCHAIN_DEFAULT_RELAY_FEE;
						//	}
						//	else
						//	{
						//		auto fee = hsrcore::client::g_client->get_wallet()->get_transaction_fee();
						//		_relay_fee = fee.amount > HSR_BLOCKCHAIN_DEFAULT_RELAY_FEE ? fee.amount : HSR_BLOCKCHAIN_DEFAULT_RELAY_FEE;
					...
					if ((now() - block_data.timestamp).to_seconds() < 10 * HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
            ...
            void ChainDatabaseImpl::pop_block()
            {
                try {
                    ...
                    hsrcore::blockchain::PendingChainStatePtr undo_state_ptr = std::make_shared<hsrcore::blockchain::PendingChainState>(undo_state);
        ...
        {
            return get_delegates_by_vote(0, HSR_BLOCKCHAIN_NUM_DELEGATES);
        }
                    ...
                    else
                    {
                        ...
                        const auto insert_block = [&](const FullBlock& block)
                        {
                            if (blocks_indexed % 200 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    progress = float(blocks_indexed * HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC) / seconds;
                                }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if (last_known_block_num > HSR_BLOCKCHAIN_MAX_UNDO_HISTORY)
                                    my->_min_undo_block = last_known_block_num - HSR_BLOCKCHAIN_MAX_UNDO_HISTORY;
                        ...
                        catch (hsrcore::blockchain::store_and_index_a_seen_block&  e)
                        {
        ...
        void ChainDatabase::close()
        {
            try {
				...
				my->_block_id_to_modify.close();

                my->_transaction_id_to_entry.close();
                my->_address_to_transaction_ids.close();
                my->_hsr_input_balance_entry.close();
                my->_hsr_full_entry.close();
		...
		AccountEntry ChainDatabase::get_block_signee(const BlockIdType& block_id)const
		{
			...
			FC_ASSERT(delegate_entry.valid(), "Invalid account!");
			//FC_ASSERT(delegate_entry.valid() && delegate_entry->is_delegate(), "Invalid delegate!");
			return *delegate_entry;
		}
            ...
            const std::vector<AccountIdType>& ordered_delegates)const
        {
            try {
                ...
                auto delegate_pos = slot_number % HSR_BLOCKCHAIN_NUM_DELEGATES;
        ...
        optional<time_point_sec> ChainDatabase::get_next_producible_block_timestamp(const vector<AccountIdType>& delegate_ids)const
        {
            try {
                ...
                if (next_block_time <= now()) next_block_time += HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                auto last_block_time = next_block_time + (HSR_BLOCKCHAIN_NUM_DELEGATES * HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

                auto active_delegates = get_active_delegates();
                for (; next_block_time < last_block_time; next_block_time += HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC)
                {
            ...
            const ShareType required_fees, bool contract_vm_exec, bool skip_signature_check, bool throw_exec_exception)
        {
            try {
                ...
                catch (hsrcore::blockchain::ignore_check_required_fee_state& e)
                {
		...
		optional<fc::ripemd160> ChainDatabase::get_head_block_modify(const BlockIdType& id)const
		{
			try {
				
				return modify_lookup_by_block_id(id);;
			} FC_CAPTURE_AND_RETHROW()
		}
		...
		void ChainDatabase::store_block_modify(const BlockIdType& id, const fc::ripemd160& entry)
		{
			try {
				modify_insert_into_block_id_map(id, entry);
			} FC_CAPTURE_AND_RETHROW()
		}
        ...
        BlockForkData ChainDatabase::push_block(const FullBlock& block_data)
        {
            try {
                ...
                if (head_block_num > HSR_BLOCKCHAIN_MAX_UNDO_HISTORY &&
                    block_data.block_num <= (head_block_num - HSR_BLOCKCHAIN_MAX_UNDO_HISTORY))
                {
                    elog("block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
                        ...
                        ("head_block_num", head_block_num)("undo_history", HSR_BLOCKCHAIN_MAX_UNDO_HISTORY));
        ...
        */
        double ChainDatabase::get_average_delegate_participation()const
        {
            try {
                ...
                const auto now = hsrcore::blockchain::now();
                if (head_num < 1)
                {
                    return 0;
                }
                else if (head_num <= HSR_BLOCKCHAIN_NUM_DELEGATES)
                {
                    // what percent of the maximum total blocks that could have been produced
                    // have been produced.
                    const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                ...
                else
                {
                    ...
                    const auto starting_time = get_block_header(head_num - HSR_BLOCKCHAIN_NUM_DELEGATES).timestamp;
                    const auto expected_production = (now - starting_time).to_seconds() / HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
                    return 100 * double(HSR_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
                }
            } FC_RETHROW_EXCEPTIONS(warn, "")
        }
        ...
        TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction(const SignedTransaction& trx, bool override_limits, bool contract_vm_exec, bool cache)
        {
            try {
                ...
                if (!override_limits)
                {
                    if (my->_pending_fee_index.size() > HSR_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE)
                    {
                        auto overage = my->_pending_fee_index.size() - HSR_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
		...
		std::vector<SignedTransaction> ChainDatabase::get_pending_transactions_original()const
		{
			...
			for (const auto& item : my->_pending_fee_index)
			{
				trxs.push_back(item.second->trx);
		...
		static const int64_t nTargetTimespan = 16 * HSR_PRODUCTE_BLOCK_INTERAL;  // 16 mins




		SignedBlockHeader ChainDatabase::GetLastBlockHeader(BlockIdType pindexLast, bool fProofOfStake)
		{
			try {
				auto temp_last_block = get_block_header(pindexLast);
				while (temp_last_block.previous != BlockIdType() && (temp_last_block.is_coinstake != fProofOfStake))
					temp_last_block = get_block_header(temp_last_block.previous);
				return temp_last_block;
			}
			catch (...)
			{
...
//		}
        ...
        FullBlock ChainDatabase::generate_block(const time_point_sec block_timestamp, Address coin_base, const DelegateConfig& config, uint32_t is_multisig_account, bool is_proof_of_stake)
        {
            try {
                ...
                //if( pending_state->get_head_block_num() >= HSR_V0_4_9_FORK_BLOCK_NUM )
                ...
                if (config.block_max_transaction_count > 0 && config.block_max_size > block_size)
                {
                    ...
                    const vector<SignedTransaction> pending_trx = get_pending_transactions_original();
                    for (const SignedTransaction& item : pending_trx)
                    {
                        // Check block production time limit
                        if (time_point::now() - start_time >= config.block_max_production_time)
                            break;

                        const SignedTransaction& new_transaction = item;
                        try
                        {
                            ...
                            auto pending_trx_state = origin_trx_state;
                            {
                                ...
                                if (trx_eval_state->p_result_trx.operations.size() > 0)
                                {
                                    ...
                                    if (iMessageLength > HSR_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
                                    {
                                        imessage_fee = config.transaction_imessage_min_fee_coe * (iMessageLength - HSR_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
                                    }
                ...
                // if( new_block.block_num < HSR_V0_7_0_FORK_BLOCK_NUM )
        ...
        string ChainDatabase::export_fork_graph(uint32_t start_block, uint32_t end_block, const fc::path& filename)const
        {
            ...
            for (auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr)
            {
                ...
                {
                    unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
        ...
        bool ChainDatabase::store_account_entries_for_sandbox(const vector<hsrcore::wallet::WalletAccountEntry>& vec_account_entry)
        {
        ...
        vector<HSRTrxidBalance> ChainDatabase::fetch_hsr_input_balance(const uint32_t & block_num)
        {
            try{
                vector < HSRTrxidBalance > results;
                for (auto iter = my->_hsr_input_balance_entry.unordered_begin();
                    iter != my->_hsr_input_balance_entry.unordered_end(); ++iter)
                {
                    HSRTrxidBalance hsrTemp;
                    hsrTemp.block_num = block_num + 1;
                    set<HSRTrxidBalance>::iterator block_iter = iter->second.lower_bound(hsrTemp);
        ...
        vector<HSRTrxidBalance> ChainDatabase::fetch_hsr_full_entry(const uint32_t& block_num, const uint32_t & last_scan_block_num)
        {
            try{
                vector < HSRTrxidBalance > results;
                ...
                for (auto iter = my->_hsr_full_entry.unordered_begin();
                    iter != my->_hsr_full_entry.unordered_end(); ++iter)
                {
                    auto block_iter = iter->second.hsr_block_sort.lower_bound(block_num + 1);
                    for (; block_iter != iter->second.hsr_block_sort.end(); block_iter++)
                    {
        ...
        void ChainDatabase::transaction_insert_to_hsr_full_entry(const string & hsr_account, const HSRTrxidBalance & hsr_balance_record)
        {
            auto iter_entry = my->_hsr_full_entry.unordered_find(hsr_account);
            HSRBalanceEntry hsrentrys;
            if (iter_entry != my->_hsr_full_entry.unordered_end())
            {
                hsrentrys = iter_entry->second;
                if (hsrentrys.hsr_trxid_sort.count(hsr_balance_record.trx_id) != 0)
                {
                    return;
                }
            }
            hsrentrys.hsr_trxid_sort.insert(hsr_balance_record.trx_id);
            hsrentrys.hsr_block_sort.insert(std::make_pair(hsr_balance_record.block_num, hsr_balance_record));
            my->_hsr_full_entry.store(hsr_account, hsrentrys);
        }
        void ChainDatabase::transaction_erase_from_hsr_full_entry(const string & hsr_account, const HSRTrxidBalance & hsr_balance_record)
        {
            auto iter_entry = my->_hsr_full_entry.unordered_find(hsr_account);
            if (iter_entry != my->_hsr_full_entry.unordered_end())
            {
                HSRBalanceEntry hsrentrys = iter_entry->second;
                hsrentrys.hsr_trxid_sort.erase(hsr_balance_record.trx_id);
                auto iter = hsrentrys.hsr_block_sort.lower_bound(hsr_balance_record.block_num);
                for (; iter != hsrentrys.hsr_block_sort.end();)
                {
                    if (iter->second.block_num != hsr_balance_record.block_num)
                    {
                        break;
                    }
                    if (iter->second.trx_id == hsr_balance_record.trx_id)
                    {
                        iter = hsrentrys.hsr_block_sort.erase(iter);
                    }
                    else
                    {
                        ++iter;
                ...
                if (!(hsrentrys.hsr_block_sort.empty()))
                {
                    my->_hsr_full_entry.store(hsr_account, hsrentrys);
                }
                else
                {
                    my->_hsr_full_entry.remove(hsr_account);
                }
            }
        }
        void ChainDatabase::transaction_insert_to_hsr_balance(const string & hsr_account, const HSRTrxidBalance & hsr_balance_entry)
        {
            auto iter_ids = my->_hsr_input_balance_entry.unordered_find(hsr_account);
            set<HSRTrxidBalance> ids;
            if (iter_ids != my->_hsr_input_balance_entry.unordered_end())
            {
                ids = iter_ids->second;
            }
            ids.insert(hsr_balance_entry);
            my->_hsr_input_balance_entry.store(hsr_account, ids);
        }
        void ChainDatabase::transaction_erase_from_hsr_balance(const string & hsr_account, const HSRTrxidBalance & hsr_balance_entry)
        {
            auto iter_ids = my->_hsr_input_balance_entry.unordered_find(hsr_account);
            if (iter_ids != my->_hsr_input_balance_entry.unordered_end())
            {
                auto ids = iter_ids->second;
                ids.erase(hsr_balance_entry);
                if (!(ids.empty()))
                {
                    my->_hsr_input_balance_entry.store(hsr_account, ids);
                }
                ...
                {
                    my->_hsr_input_balance_entry.remove(hsr_account);
                }
            }
        }
		...
		bool ChainDatabase::CheckProofOfStake(SignedBlockHeader& pblock_header, int nTimeSearch)
		{
			try
			{
				CBigNum bnTarget;
				...
				int success = 0;
				auto cur_block = get_head_block();
				if (pblock_header.previous != cur_block.id())
					return false;
				fc::ripemd160 modify_value;
				if (pblock_header.block_num == 1)
				{
					modify_value = fc::ripemd160();
				}
				else
				{
					auto omodify = get_head_block_modify(pblock_header.previous);
					if (!omodify.valid())
						return false;
					modify_value = *omodify;
				}
				//last block modify value
				...
				TransactionLocation trx_location;
				auto trx_entry = get_transaction(temp_trx_id);
				if (!trx_entry.valid())
				{

					auto block_entry = get_block(temp_trx_id);

					trx_location.block_num = block_entry.block_num;
					trx_location.trx_num = -1;
				}
				else
					trx_location = trx_entry->chain_location;

				if (get_head_block_num() - trx_location.block_num < HSR_POS_BALANCE_PRODUCTE_INTERAL)
					return false;

				temp_data = fc::raw::pack(temp_trx_id);
				data.insert(data.end(), temp_data.begin(), temp_data.end());
				temp_data = fc::raw::pack(trx_location.block_num);
				data.insert(data.end(), temp_data.begin(), temp_data.end());
				temp_data = fc::raw::pack(trx_location.trx_num);
				data.insert(data.end(), temp_data.begin(), temp_data.end());

				for (int i = 0; i < nTimeSearch; ++i)
				{
					vector<char> res_data;
					res_data.insert(res_data.end(), data.begin(), data.end());
					temp_data = fc::raw::pack(pblock_header.timestamp - i);
					res_data.insert(res_data.end(), temp_data.begin(), temp_data.end());
					uint256 res = HashR14(res_data.begin(), res_data.end());
					if (res <= nTarget)
					{
						success = 1;
						pblock_header.timestamp -= i;
						break;
					}
				}
				...
				if (success)
				{
					auto aa = get_head_block().id();
					if (pblock_header.block_num == 1)
					{
						FC_ASSERT(pblock_header.previous == BlockIdType(), "CheckWork() : generated block is stale");
					}
					else
						FC_ASSERT(pblock_header.previous == get_head_block().id(), "CheckWork() : generated block is stale");
					return true;
				}

			}
			FC_CAPTURE_AND_LOG(("checkProofError"));
			return false;
		...
		bool ChainDatabase::CheckStake(SignedBlockHeader& pblock)
		{
			uint256 proofHash = 0, hashTarget = 0;
			BlockIdType hashBlock = pblock.id();

			if (pblock.is_coinstake != 1)
			{
			...
			fc::time_point_sec nLastSearchTime = nLastSearchTime_map[pblock.coin_base];
			fc::time_point_sec cur_time = fc::time_point::now();
			pblock.timestamp = cur_time;
			if (cur_time > nLastSearchTime)
			{
				
				if (!CheckProofOfStake(pblock, (cur_time - nLastSearchTime).to_seconds()))
				{
					wlog("CheckStake() : proof - of - stake checking failed");
					nLastSearchTime = cur_time;
					return false;
				}
				nLastSearchTime_map[pblock.coin_base] = cur_time;
				// Found a solution
				{
					if (pblock.previous != get_head_block_id())
						return false;
				}
        ...
        void ChainDatabase::dump_state(const fc::path& path)const
        {
            try {
                ...
                */
                ...
                ulog("Dumped ${p}", ("p", next_path));
                next_path = dir / "hsr_transaction_balance_db.json";
                my->_hsr_input_balance_entry.export_to_json(next_path);
				...
				next_path = dir / "_block_id_to_modify.json";
				my->_block_id_to_modify.export_to_json(next_path);
        ...
        void hsrcore::blockchain::ChainDatabase::contract_store_resultid_by_reqestid(const TransactionIdType & req, const ResultTIdEntry & res)
        {
            my->_request_to_result_iddb.store(req, res);
        }

        void hsrcore::blockchain::ChainDatabase::contract_erase_resultid_by_reqestid(const TransactionIdType & req)
        {
            my->_request_to_result_iddb.remove(req);
        }

        oRequestIdEntry hsrcore::blockchain::ChainDatabase::contract_lookup_requestid_by_resultid(const TransactionIdType &id) const
        {
        ...
        void hsrcore::blockchain::ChainDatabase::contract_store_requestid_by_resultid(const TransactionIdType & res, const RequestIdEntry & req)
        {
            my->_result_to_request_iddb.store(res, req);
        }

        void hsrcore::blockchain::ChainDatabase::contract_erase_requestid_by_resultid(const TransactionIdType & result)
        {
            my->_result_to_request_iddb.remove(result);
        }

        oContractinTrxEntry hsrcore::blockchain::ChainDatabase::contract_lookup_contractid_by_trxid(const TransactionIdType &id) const
        {
        ...
        oContractTrxEntry hsrcore::blockchain::ChainDatabase::contract_lookup_trxid_by_contract_id(const ContractIdType &id) const
        {
        ...
        void hsrcore::blockchain::ChainDatabase::contract_store_contractid_by_trxid(const TransactionIdType & tid, const ContractinTrxEntry & cid)
        {
            my->_trx_to_contract_iddb.store(tid, cid);
        }

        void hsrcore::blockchain::ChainDatabase::contract_store_trxid_by_contractid(const ContractIdType & cid, const ContractTrxEntry & tid)
        {
            my->_contract_to_trx_iddb.store(cid, tid);
        }

        void hsrcore::blockchain::ChainDatabase::contract_erase_trxid_by_contract_id(const ContractIdType &id)
        {
            my->_contract_to_trx_iddb.remove(id);
        }

        void hsrcore::blockchain::ChainDatabase::contract_erase_contractid_by_trxid(const TransactionIdType & tid)
        {
		...
		optional<fc::ripemd160> ChainDatabase::modify_lookup_by_block_id(const BlockIdType& id)const
		{
			return my->_block_id_to_modify.fetch_optional(id);
		}

		void ChainDatabase::modify_insert_into_block_id_map(const BlockIdType& id , const fc::ripemd160& entry)
		{
			my->_block_id_to_modify.store(id,entry);
		}

		void ChainDatabase::modify_erase_from_block_id_map(const BlockIdType& id)
		{
			my->_block_id_to_modify.remove(id);
		}
