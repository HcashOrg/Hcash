...
namespace hsrcore {
    namespace blockchain {
            ...
            bool use_stealth_address)
        {
            ...
            if (memo_message.size() <= HSR_BLOCKCHAIN_MAX_MEMO_SIZE)
            {
    ...
    void from_variant(const variant& var, hsrcore::blockchain::MemoData& vo)
    {
        try {
            ...
            if (obj.contains("from"))
                vo.from = obj["from"].as<hsrcore::blockchain::PublicKeyType>();
            ...
            if (obj.contains("memo_flags"))
                vo.memo_flags = obj["memo_flags"].as<hsrcore::blockchain::MemoFlagsEnum>();
