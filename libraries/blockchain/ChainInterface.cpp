...
namespace hsrcore {
    namespace blockchain {
        ...
        {
            try {
                if (name.size() < HSR_BLOCKCHAIN_MIN_NAME_SIZE) return false;
                if (name.size() > HSR_BLOCKCHAIN_MAX_NAME_SIZE) return false;
        ...
        {
            try {
                // if( get_head_block_num() < HSR_V0_5_0_FORK_BLOCK_NUM )
                // return is_valid_symbol_name_v1( symbol );

                FC_ASSERT(symbol != "HSR", "Symbol must not be HSR");

                if (symbol.size() < HSR_BLOCKCHAIN_MIN_SYMBOL_SIZE)
                ...
                if (dots == 0)
                {
                    if (position > HSR_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }
                else //dots == 1 means hierarchial asset name
                {
                    if (position - dot_position - 1 > HSR_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
                        FC_ASSERT(false, "Sub-symbol name too large");
                    if (symbol.size() > HSR_BLOCKCHAIN_MAX_SYMBOL_SIZE)
                        FC_ASSERT(false, "Symbol name too large");
                }

                if (symbol.find("HSR") == 0)
                    FC_ASSERT(false, "Symbol names cannot be prefixed with HSR");
        ...
        ShareType ChainInterface::get_max_delegate_pay_issued_per_block(bool is_coin_stake)const
        {
			//return ShareType(0);
			int count = get_head_block_num() / 525600;
			if (get_head_block_num() < 52560000)
				return 1.6 * HSR_BLOCKCHAIN_PRECISION - 0.016 * HSR_BLOCKCHAIN_PRECISION * count;
			else
			{
				if (is_coin_stake)
					return HSR_BLOCKCHAIN_PRECISION;
				else
					return 0;
			}

            ShareType pay_per_block = HSR_MAX_DELEGATE_PAY_PER_BLOCK;
        ...
        {
            // if( get_head_block_num() < HSR_V0_4_24_FORK_BLOCK_NUM )
            ...
            const ShareType max_total_pay_per_two_weeks = blocks_per_two_weeks * ShareType(500) * int64_t(HSR_BLOCKCHAIN_PRECISION);
            const ShareType max_pay_per_two_weeks = max_total_pay_per_two_weeks / HSR_BLOCKCHAIN_NUM_DELEGATES;
        ...
        {
            if (imessage.size() < HSR_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE)
            {
                return 0;
            }
            ImessageIdType fee_coe = HSR_BLOCKCHAIN_MIN_MESSAGE_FEE_COE;
            ShareType fee = 0;
            fee = fee_coe * (imessage.size() - HSR_BLOCKCHAIN_MAX_FREE_MESSAGE_SIZE);
            return fee;
        }
        ShareType ChainInterface::get_asset_registration_fee(uint8_t symbol_length)const
        {
            ...
            static const ShareType long_symbol_price = ShareType(500) * HSR_BLOCKCHAIN_PRECISION; // $10 at $0.02/XTS
        ...
        double ChainInterface::to_pretty_price_double(const Price& price_to_pretty_print)const
        {
			...
			//change for outof range
            return fc::variant(string(price_to_pretty_print.ratio * obase_asset->precision / oquote_asset->precision)).as_double() / (ShareType(HSR_BLOCKCHAIN_MAX_SHARES));
        }
        ...
        uint64_t ChainInterface::get_required_confirmations()const
        {
            try {
                ...
                return HSR_BLOCKCHAIN_NUM_DELEGATES * 3;
            } FC_CAPTURE_AND_RETHROW()
        }
        ...
        hsrcore::blockchain::Asset ChainInterface::get_amount(ShareType limit, AssetIdType asset_id)
        {
        ...
        {
            Asset fee(HSR_DEFAULT_TRANSACTION_FEE, 0);
            return fee;
        }
        ...
        {
            Asset fee(HSR_DEFAULT_CONTRACT_MARGIN, 0);
            return fee;
        }
        ...
        {
            return HSR_DEFAULT_TRANSACTION_EXPIRATION_SEC;
        }
        */
            ...
            const Address& to_account_address, SignedTransaction& trx)
        {
            try {
				...
				{
					FC_CAPTURE_AND_THROW(invalid_asset_symbol,("Only HSR Allowed"));
				}
