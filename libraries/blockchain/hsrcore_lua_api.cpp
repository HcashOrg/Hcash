/**
 * lua module injector header in hsrcore
 */
    ...
    namespace lua {
        namespace api {
            ...
            */
            ...
            /**
            * when exception happened, use this api to tell hsrcore
            ...
            */
            ...
            {
                return 0; // FIXME: need fill by hsrcore api
            }
            ...
            {
                blockchain::TransactionEvaluationState* pevaluate_state = (blockchain::TransactionEvaluationState*)lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value;
                blockchain::oContractEntry entry = pevaluate_state->_current_state->get_contract_entry(hsrcore::blockchain::Address(std::string(address), AddressType::contract_address));
            ...
            void GluaChainApi::get_contract_address_by_name(lua_State *L, const char *name, char *address, size_t *address_size)
            {
                ...
                */
                hsrcore::blockchain::TransactionEvaluationState* eval_state_ptr =
                    (hsrcore::blockchain::TransactionEvaluationState*)
                    (hsrcore::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
                ...
                hsrcore::blockchain::ChainInterface* cur_state = eval_state_ptr->_current_state;
            ...
            bool GluaChainApi::check_contract_exist_by_address(lua_State *L, const char *address)
            {
                ...
                oContractEntry entry = cur_state->get_contract_entry(hsrcore::blockchain::Address(std::string(address), AddressType::contract_address));
            ...
            {
                /*
                char *filename = lutil_concat_str4("hsrcore_lua_modules", file_separator_str(), "hsrcore_contract_", name);
            ...
            /**
            * load contract lua byte stream from hsrcore api
            */
            ...
            /**
            * store contract lua module byte stream to hsrcore api
            */
            //need to delete
            /*
            ...
            */
            ...
            GluaStorageValue GluaChainApi::get_storage_value_from_hsrcore(lua_State *L, const char *contract_name, std::string name)
            {
                ...
                return get_storage_value_from_hsrcore_by_address(L, entry->id.AddressToString(AddressType::contract_address).c_str(), name);
            }

            GluaStorageValue GluaChainApi::get_storage_value_from_hsrcore_by_address(lua_State *L, const char *contract_address, std::string name)
            {
                GluaStorageValue null_storage;
                null_storage.type = hsrcore::blockchain::StorageValueTypes::storage_value_null;
                ...
                hsrcore::blockchain::StorageDataType storage_data = iter->second;

                return hsrcore::blockchain::StorageDataType::create_lua_storage_from_storage_data(L, storage_data);
            }

            bool GluaChainApi::commit_storage_changes_to_hsrcore(lua_State *L, AllContractsChangesMap &changes)
            {
                hsrcore::blockchain::TransactionEvaluationState* eval_state_ptr =
                    (hsrcore::blockchain::TransactionEvaluationState*)
                    (hsrcore::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
            ...
            {
                // TODO
                printf("registered storage %s[%s] to hsrcore\n", contract_name, name);
                return true;
            }

            intptr_t GluaChainApi::register_object_in_pool(lua_State *L, intptr_t object_addr, GluaOutsideObjectTypes type)
			{
				auto node = hsrcore::lua::lib::get_lua_state_value_node(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY);
				...
				if (node.type == GluaStateValueType::LUA_STATE_VALUE_nullptr)
				{
					...
					hsrcore::lua::lib::set_lua_state_value(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY, node.value, node.type);
				}
				else
				{
            ...
            void GluaChainApi::release_objects_in_pool(lua_State *L)
			{
				...
				for (const auto &p : *object_pools)
				{
					...
					for (const auto &object_item : *pool)
					{
						...
						{
							auto stream = (hsrcore::lua::lib::GluaByteStream*) object_addr;
							delete stream;
						} break;
						default: {
							continue;
				...
				hsrcore::lua::lib::set_lua_state_value(L, GLUA_OUTSIDE_OBJECT_POOLS_KEY, null_state_value, GluaStateValueType::LUA_STATE_VALUE_nullptr);
			}
              ...
              hsrcore::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                ...
                try
                {
                    if (!Address::is_valid(contract_address, CONTRACT_ADDRESS_PREFIX))
                        return -3;
                    if (!Address::is_valid(to_addr, HSR_ADDRESS_PREFIX))
                        return -4;

                    eval_state_ptr->transfer_asset_from_contract(amount, asset_type,

                        Address(string(contract_address), AddressType::contract_address), Address(to_addr, AddressType::hsr_address));
              ...
              hsrcore::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    hsrcore::blockchain::TransactionEvaluationState* eval_state_ptr =
                        (hsrcore::blockchain::TransactionEvaluationState*)
                        (hsrcore::lua::lib::get_lua_state_value(L, "evaluate_state").pointer_value);
                    hsrcore::blockchain::ChainInterface* cur_state;
                    ...
                    {
                        FC_CAPTURE_AND_THROW(unknown_asset_id, ("Only HSR Allowed"));
                    }
            ...
            int32_t GluaChainApi::get_waited(lua_State *L, uint32_t num)
            {
              hsrcore::lua::lib::increment_lvm_instructions_executed_count(L, CHAIN_GLUA_API_EACH_INSTRUCTIONS_COUNT - 1);
                try{
                    ...
                    SecretHashType _hash = fc::ripemd160::hash(_header.nNonce);
                    ...
                    for (int i = 0; i < 50; i++)
                    {
                        ...
                        _hash = _hash.hash(_header.nNonce);
                    }
			...
			bool GluaChainApi::is_valid_address(lua_State *L, const char *address_str)
			{
