    ...
    namespace blockchain
    {
        ...
        ShareType check_balances(TransactionEvaluationState& eval_state, const std::map<BalanceIdType, ShareType>& balances_check, const Address& address_check)
        {
            ...
            while (it != balances_check.end())
            {
                ...
                FC_ASSERT(current_balance_entry->asset_id() == 0, "Asset must use HSR");
            ...
            try
            {
                ...
                if (!has_on_upgrade)
                {
                    ...
                    if (!eval_state.evaluate_contract_testing)
                    {
                        ...
                        eval_state.required_fees = transaction_fee + Asset(HSR_DEFAULT_CONTRACT_MARGIN, 0);
                    }
                }
                else  // has on upgrade function
                {
                    ...
                    if (!eval_state.skipexec)
                    {
                        ...
                        try
                        {
							...
							{
								hsrcore::blockchain::contract_error con_err(32000, "exception", e.what());
								throw con_err;
							}
        ...
        void CallContractOperation::evaluate(TransactionEvaluationState& eval_state) const
        {
            try {
                ...
                if (entry->state == hsrcore::blockchain::ContractState::deleted)
        ...
        void TransferContractOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
                ...
                FC_ASSERT(transfer_amount.asset_id == 0, "transfer asset must be HSR");
                FC_ASSERT(costlimit.asset_id == 0, "Execution of contract must cost HSR");
