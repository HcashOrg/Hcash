...
namespace hsrcore {
    namespace blockchain {
        ...
        {
            try {
				if (eval_state.deposit_count > HSR_BLOCKCHAIN_TRANSACTION_MAX_DEPOSIT_NUM)
				...
				if(this->amount > HSR_MIN_CHANGE_BALANCE_UPDATE_TIME)
					cur_entry->last_update_transaction_id = eval_state.cur_trx_id;
                ...
                // if( eval_state._current_state->get_head_block_num() >= HSR_V0_6_0_FORK_BLOCK_NUM )
                //{
        ...
        void WithdrawOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
				...
				current_balance_entry->last_update_transaction_id = eval_state.cur_trx_id;
        ...
        void WithdrawContractOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
                ...
                FC_ASSERT(current_balance_entry->condition.asset_id == 0, "Invalid balance_id, asset type should be HSR");
                auto asset_rec = eval_state._current_state->get_asset_entry(current_balance_entry->condition.asset_id);
                FC_ASSERT(asset_rec.valid(), "Invalid asset entry");
        ...
        void BalancesWithdrawOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
                ...
                while (it != balances.end())
                {
                    ...
                    FC_ASSERT(current_balance_entry->asset_id() == 0, "BalancesWithdrawOperation asset must use HSR");
        ...
        void UpdateBalanceVoteOperation::evaluate(TransactionEvaluationState& eval_state)const
        {
            try {
                ...
                FC_ASSERT(current_balance_entry->condition.asset_id == 0, "Only HSR balances can have restricted owners.");
                ...
                auto fee = HSR_BLOCKCHAIN_PRECISION / 2;
                ...
                else // NOT this->new_restricted_owner.valid() || (this->new_restricted_owner == new_restricted_owner)
                {
                    ...
                    */
                    ...
                    FC_ASSERT(eval_state._current_state->now().sec_since_epoch() - last_update_secs
                        >= HSR_BLOCKCHAIN_VOTE_UPDATE_PERIOD_SEC,
