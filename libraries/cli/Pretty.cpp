...
namespace hsrcore {
    namespace cli {
        ...
        string pretty_delegate_list(const vector<AccountEntry>& delegate_entrys, ClientRawPtr client)
        {
            ...
            const auto next_slot_time = std::max(current_slot_timestamp, head_block_timestamp + HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
            const auto& active_delegates = client->get_chain()->get_active_delegates();
			const auto next_slot_signee = *client->get_chain()->get_account_entry(active_delegates[0]);//client->get_chain()->get_slot_signee(next_slot_time, active_delegates);
        ...
        string pretty_block_list(const vector<BlockEntry>& block_entrys, ClientRawPtr client)
        {
            ...
            for (const auto& BlockEntry : block_entrys)
            {
                ...
                {
                    last_block_timestamp -= HSR_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
