...
namespace fc { namespace http {
  ...
  class server::impl 
  {
    public:
      ...
      impl(const fc::ip::endpoint& p ,std::vector<std::string>allow_ips)
      {
        tcp_serv.set_reuse_address();
        tcp_serv.listen(p);
        accept_complete = fc::async([this, allow_ips](){ this->accept_loop(allow_ips); }, "http_server accept_loop");
      }
      ...
      void accept_loop(std::vector<std::string> allow_ips) 
      {
  ...
  server::server( uint16_t port ) :my( new impl(fc::ip::endpoint( fc::ip::address(),port), std::vector<std::string>()) ){}
  ...
  void server::listen( const fc::ip::endpoint& p,std::vector<std::string>allow_ips)
  {
    my.reset( new impl(p, allow_ips) );
  }
