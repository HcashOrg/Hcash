...
namespace fc {
  ...
  void* thread_ptr() {
   ...
   void thread::test_quit()
   {
       printf("task_pqueue count:%d\n", my->task_pqueue.size());
       ...
       printf("task_sch_queue count:%d\n", my->task_sch_queue.size());
       ...
       printf("sleep_pqueue count:%d\n", my->sleep_pqueue.size());
       ...
       printf("ready_heap count:%d\n", my->ready_heap.size());
       for (fc::context* ready_context : my->ready_heap)
           ready_context->canceled = true;
       for (auto ready : my->ready_heap)
       {
           
           if (ready->cur_task != nullptr)
           {
               printf("ready desc : %s\n", ready->cur_task->get_desc());

           }
           else
               printf("ready task null");
       }
  ...
  void thread::quit() 
  {
    ...
    for (fc::context* ready_context : my->ready_heap)
        my->free_list.push_back(ready_context);
    my->ready_heap.clear();
