...
namespace fc { namespace rpc {
   ...
   namespace detail
   {
      class json_connection_impl 
      {
         public:
            ...
            fc::future<void>                                                      _queue_handle;
            std::list<fc::future<void>>                                           _future_list;
            std::queue<fc::variant_object>                                        _all_message_queue;
            ...
            void read_loop()
            {
               ...
               try 
               {
                  ...
                  while( !_done.canceled() )
                  {
                      ...
                      //_all_message_queue.push(v.get_object());  //异步缓冲方式
            ...
            void handle_loop()
            {
                ...
                try
                {
                    ...
                    fc::string line;
                    while (!_queue_handle.canceled())
                    {
                      //  printf("all_message_queue size %d _future_list size %d\n", _all_message_queue.size(), _future_list.size());
                        if (_all_message_queue.size() == 0)
                        {
                            if (_future_list.size() > 0)
                            {
                                for (auto future_iter = _future_list.begin();
                                    future_iter != _future_list.end();)
                                {
                                    if (future_iter->ready())
                                    {
                                        future_iter = _future_list.erase(future_iter);
                                    }
                                    else
                                        ++future_iter;
                                }
                        ...
                        if (_future_list.size() < 50)
                        {
                            fc::variant_object v_obj = _all_message_queue.front();
                            
                            _handle_message_future = fc::async([=](){ handle_message(v_obj); }, "json_connection handle_message");
                            _future_list.emplace_back(_handle_message_future);
                            _all_message_queue.pop();
                        }
                        else
                        {
                            for (auto future_iter = _future_list.begin();
                            ...
                            if (_future_list.size() == 50)
                            {
                                _future_list.begin()->wait();
                                _future_list.erase(_future_list.begin());
                            }
                        }
                    }
                }
   ...
   fc::future<void> json_connection::exec()
   {
    ...
    //异步缓冲方式//  my->_queue_handle = fc::async([=](){ my->handle_loop(); }, "json_connection handle_loop");
   ...
   void json_connection::close()
   {
      try
      {
         ...
         if (my->_queue_handle.valid() && !my->_queue_handle.ready())
         {
             my->_queue_handle.cancel("json_connection is destructing");
             my->_queue_handle.wait();
         }
      }
