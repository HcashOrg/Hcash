...
namespace fc { namespace ecc {
	...
	extern "C" EC_GROUP *new_ec_group(int is_prime_field,// «∑Ò «Àÿ ˝”Ú
    ...
    {
        private_key self;
		self.my->_key = EC_KEY_new();
		EC_KEY_set_group(self.my->_key,private_key::m_pgroup);
    ...
    {
       public_key pub;
	   pub.my->_key = EC_KEY_new();
	   EC_KEY_set_group(pub.my->_key, private_key::m_pgroup);
        ...
        try {
            ...
            while( true )
            {
                ...
                if (nBitsR <= 256 && nBitsS <= 256)
                {
					...
					EC_KEY_set_group(key, private_key::m_pgroup);
                    FC_ASSERT( key );
