#include <bit/blockchain/address.hpp>
#include <bit/blockchain/genesis_state.hpp>
#include <bit/blockchain/config.hpp>
#include <bit/blockchain/pts_address.hpp>
...
using namespace bit::blockchain;
...
//   return 0;
  ...
  config.initial_balances.reserve(9);
  std::vector<fc::ecc::private_key> keys;
  keys.reserve(9);

  fc::time_point_sec time(1508250811);
  ...
  for (unsigned i = 0; i < 9; ++i)
  {
	  ...
	  if (i == 0) {
		  balan.balance = 265000000000000000;
		  balan.raw_address = string(bit::blockchain::address(keys[i].get_public_key()));
